...
PROVIDER_ORDER = [x.strip().lower() for x in
                  (os.getenv("DATA_PROVIDER_ORDER") or
                   "sifting,twelvedata,finnhub,alphavantage,yahoo2,yahoo").split(",")
                  if x.strip()]
...
def health():
    ...
    return {"status": "ok", "version": "4.4", "bot_mode": BOT_MODE, "providers": provider,
...
def provider_status():
    now = time.time(); out=[]
    configured=[("SiftingIO", "sifting", SIFTING_API_KEY), ("TwelveData", "twelvedata", TWELVEDATA_API_KEY), ("Finnhub", "finnhub", FINNHUB_API_KEY), ("Alpha Vantage", "alphavantage", ALPHAVANTAGE_API_KEY), ("Yahoo fallback", "yahoo", True)]
