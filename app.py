...
ACCESS_CODE = (os.getenv("NEXCANDLE_ACCESS_CODE") or "2580").strip()
...
# FX weekly session guard. Uses New York timezone so DST is handled automatically.
# The public UI may be viewed from any timezone; all session calculations remain
# anchored to the real FX weekly session in New York.
...
def _fx_open_time():
    return datetime.strptime(f"{_FX_OPEN_HOUR:02d}:{_FX_OPEN_MINUTE:02d}","%H:%M").time()

def _fx_close_time():
    return datetime.strptime(f"{_FX_CLOSE_HOUR:02d}:{_FX_CLOSE_MINUTE:02d}","%H:%M").time()
...
def fx_market_status():
    ...
    op=_fx_open_time()
    cl=_fx_close_time()
    # Saturday + Sunday are closed. Friday closes before the weekly break.
    if wd>=5 or (wd==4 and t>=cl):
        return False,n
    if t>=op or t<cl:
        return True,n
    return False,n
...
def fx_next_open():
    ...
    op=_fx_open_time()
    days=(6-n.weekday())%7
    if days==0 and n.time()>=op:
        days=7
    d=n+timedelta(days=days)
    return d.replace(hour=_FX_OPEN_HOUR,minute=_FX_OPEN_MINUTE,second=0,microsecond=0)

def fx_current_session_open():
    """Return the most recent weekly open in New York time, or None while closed."""
    opened,n=fx_market_status()
    if not opened:
        return None
    if n.weekday()==6:
        return n.replace(hour=_FX_OPEN_HOUR,minute=_FX_OPEN_MINUTE,second=0,microsecond=0)
    # During Monday-Friday, the current session opened on the preceding Sunday.
...
def market_status_api():
    opened,n=fx_market_status()
    if opened:
        return jsonify({"ok":True,"open":True,"message":"FOREX MARKET OPEN","server_time":n.isoformat()})
    return jsonify({"ok":True,"open":False,"message":"FOREX MARKET CLOSED",
                    "next_open":fx_next_open().isoformat(),"server_time":n.isoformat()})

@web.get("/")
def home():
    return Response(open(os.path.join(os.path.dirname(__file__), "index.html"), "r", encoding="utf-8").read(),
                    mimetype="text/html")
...
def premium_signal_api():
    ...
    try:
        # After the weekend, never reuse Friday/cached candles for a Monday
        # signal. Wait until at least one completed candle belongs to the new
        # weekly session.
        ready, last_candle = _session_candle_ready(pair, tf)
        if not ready:
            return jsonify({
                "ok": True,
                "market_open": True,
                "signal": None,
                "waiting_for_fresh_session_candle": True,
                "message": "WAITING FOR FRESH MARKET CANDLE",
            })
