...
## v3.7 SiftingIO Advanced Market Data
SiftingIO is the preferred Forex provider when `SIFTING_API_KEY` is configured. It provides historical FX OHLCV via REST and live FX ticks via WebSocket. The WebSocket uses one connection and up to five symbols on the free tier; the client sends keepalive pings and reconnects with backoff.

Render: add `SIFTING_API_KEY` with your private key. Do not paste the key into Telegram/chat.
