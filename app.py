...
def premium_signal_api():
    ...
    try:
        ...
        # Last-resort directional path: if MTF fusion is temporarily unavailable
        # but a fresh validated candle feed is available, still derive the public
        # UP/DOWN direction from the local multi-factor engine. This keeps the
        # premium UI directional without fabricating a result.
        try:
            x = candles(pair, tf)
            ...
            if direction == "WAIT":
                a = calc(x).iloc[-1]
                p = calc(x).iloc[-2]
                direction = "CALL" if (a.close > a.e21 and a.macd >= a.ms) else "PUT"
            s = {
                "direction": direction,
            ...
            s["display_direction"] = "UP" if direction == "CALL" else "DOWN"
            s["display_score"] = s["score"]
            s["display_pair"] = pair
            s["display_tf"] = tf
            return jsonify({"ok": True, "signal": s, "fallback": True})
        except Exception:
