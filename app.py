...
# ============================================================
# NexCandle AI v4.6 Hyper Next-Candle UP/DOWN Engine
# ============================================================
...
PROVIDER_ORDER = [x.strip().lower() for x in
                  (os.getenv("DATA_PROVIDER_ORDER") or
                   "biquote,dukascopy,sifting,twelvedata,finnhub,alphavantage,yahoo2,yahoo").split(",")
                  if x.strip()]
...
DATA_TIMEOUT = max(4, int(os.getenv("MARKET_DATA_TIMEOUT_SECONDS", "8")))
...
def health():
    ...
    return {"status": "ok", "version": "4.6", "bot_mode": BOT_MODE, "providers": provider,
...
def _dukascopy(pair, tf, limit):
    ...
    base_tf = "1hour" if tf == "4h" else "1min"
    count = min(5000, max(500, int(limit * (4 if tf == "4h" else max(1, TF_MIN.get(tf, 1))))))
    r = S.get(
        "https://freeserv.dukascopy.com/2.0/",
        params={
    ...
    with fetch_lock:
        ...
        def validate_and_store(name, x):
            if x is None or len(x) < 80:
                raise RuntimeError(f"insufficient candles ({0 if x is None else len(x)})")
            interval = TF_SECONDS[tf]
        ...
        if ready_public:
            with ThreadPoolExecutor(max_workers=len(ready_public)) as ex:
                futures = {ex.submit(f, pair, tf, limit): n for n, f in ready_public}
        ...
        for name, fn in providers:
            ...
            if not _provider_ready(name):
                continue
            try:
                return validate_and_store(name, fn(pair, tf, limit))
            except Exception as e:
                errors.append(f"{name}: {str(e)[:140]}")
                _provider_failed(name, e)
...
def provider_status():
    now = time.time(); out=[]
    configured=[("BiQuote", "biquote", True), ("Dukascopy", "dukascopy", DUKASCOPY_ENABLED), ("SiftingIO", "sifting", SIFTING_API_KEY), ("TwelveData", "twelvedata", TWELVEDATA_API_KEY), ("Finnhub", "finnhub", FINNHUB_API_KEY), ("Alpha Vantage", "alphavantage", ALPHAVANTAGE_API_KEY), ("Yahoo fallback", "yahoo", True)]
