...
.stage{height:500px;position:relative;display:grid;place-items:center;overflow:hidden;perspective:1000px}
.stage:before{content:"";position:absolute;width:70%;height:70%;border-radius:50%;background:radial-gradient(circle,rgba(0,255,157,.12),transparent 65%);filter:blur(10px)}
.core{width:245px;height:245px;position:relative;transform-style:preserve-3d;animation:float 4.2s ease-in-out infinite}
...
.result.up .word,.result.down .word{animation:signalIn .55s cubic-bezier(.2,.8,.2,1)}
...
.row{display:flex;align-items:center;justify-content:space-between;padding:15px 14px;border:1px solid #17343d;border-radius:16px;background:linear-gradient(145deg,#071218,#050c11);transition:.2s}
...
def get_directional_signal(pair, tf):
    """Return only CALL/PUT internally, with resilient validated fallback.

    Primary path: full MTF signal.
    Secondary path: fresh local multi-factor analysis if MTF is temporarily
    unavailable.
    Tertiary path: the last validated direction for this exact pair/timeframe
    during a short provider outage. It is never generated randomly.
    """
    key=(pair,tf)
    now=time.time()
    try:
        ...
        if direction not in {"CALL","PUT"}:
            ...
            if direction not in {"CALL","PUT"}:
                a=calc(x).iloc[-1]
                p=calc(x).iloc[-2]
                direction="CALL" if (a.close>p.close and a.close>=a.e21) else "PUT"
            s=dict(s)
