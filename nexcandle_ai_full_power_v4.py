...
Single-file production-oriented V4: NexCandle AI
...
- RSI, EMA 9/21, MACD, Bollinger Bands, ADX, ATR, Stochastic, VWAP, Fibonacci
  retracement levels
- Multi-timeframe confirmation (1m + 5m + 15m + 30m + 35m + 45m + 1h)
...
INTERVALS = {
    ...
    "30m": "30m", "35m": "35m", "45m": "45m", "1h": "1h", "60m": "1h",
    "30min": "30m", "35min": "35m", "45min": "45m", "60min": "1h",
...
def init_db():
    with DB_LOCK:
        ...
        for col in ["model_probability", "power_quality", "entry_wait_minutes"]:
            if col not in cols:
                con.execute(f"ALTER TABLE signals ADD COLUMN {col} REAL")
        for col in ["entry_time", "window_end", "timing_action", "mtf_agreement"]:
            if col not in cols:
                con.execute(f"ALTER TABLE signals ADD COLUMN {col} TEXT")
        ...
        con.execute("""
            CREATE TABLE IF NOT EXISTS subscriptions (
                ...
                order_ref TEXT,
                created_at TEXT,
                awaiting_txn INTEGER DEFAULT 0
            )
        """)
        cols_sub = [r[1] for r in con.execute("PRAGMA table_info(subscriptions)").fetchall()]
        if "awaiting_txn" not in cols_sub:
            con.execute("ALTER TABLE subscriptions ADD COLUMN awaiting_txn INTEGER DEFAULT 0")
        con.execute("""
        ...
        """)
        cols_watch = [r[1] for r in con.execute("PRAGMA table_info(watchlist_items)").fetchall()]
        if "custom_threshold" not in cols_watch:
            con.execute("ALTER TABLE watchlist_items ADD COLUMN custom_threshold INTEGER")
        con.commit()
        con.close()
...
def set_awaiting_txn(telegram_id, value: bool):
    telegram_id = str(telegram_id)
    with DB_LOCK:
        con = db()
        get_or_create_user(telegram_id)
        con.execute(
            "UPDATE subscriptions SET awaiting_txn = ? WHERE telegram_id = ?",
            (1 if value else 0, telegram_id)
        )
        con.commit()
        con.close()

def is_awaiting_txn(telegram_id):
    row = get_or_create_user(telegram_id)
    return bool(row["awaiting_txn"])
...
def get_watchlist(telegram_id):
    ...
    with DB_LOCK:
        con = db()
        rows = con.execute(
            "SELECT symbol, last_alert_at, custom_threshold FROM watchlist_items WHERE telegram_id = ? ORDER BY added_at",
            (telegram_id,)
        ).fetchall()
        con.close()
    return rows
...
def set_watchlist_threshold(telegram_id, symbol, threshold):
    telegram_id = str(telegram_id)
    symbol = normalize_symbol(symbol)
    threshold = max(MIN_ALERT_THRESHOLD, min(MAX_ALERT_THRESHOLD, int(threshold)))
    with DB_LOCK:
        con = db()
        cur = con.execute(
            "UPDATE watchlist_items SET custom_threshold = ? WHERE telegram_id = ? AND symbol = ?",
            (threshold, telegram_id, symbol)
        )
        con.commit()
...
def normalize_interval(interval: str) -> str:
    ...
    if i not in INTERVALS:
        raise ValueError("Supported intervals: 1m, 5m, 15m, 30m, 35m, 45m, 1h, 30m, 35m, 45m, 1h")
    return INTERVALS[i]

CUSTOM_INTERVAL_MINUTES = {"30m": 30, "35m": 35, "45m": 45, "1h": 60}

MARKET_DATA_CACHE = {}
MARKET_DATA_CACHE_LOCK = threading.Lock()
MARKET_CACHE_TTL_SECONDS = float(os.getenv("MARKET_CACHE_TTL_SECONDS", "5"))

def _provider_fetch_candles(symbol: str, interval: str, outputsize: int = 250) -> pd.DataFrame:
    cache_key = (normalize_symbol(symbol), str(interval), int(outputsize))
    now_epoch = __import__("time").time()
    with MARKET_DATA_CACHE_LOCK:
        cached = MARKET_DATA_CACHE.get(cache_key)
        if cached and now_epoch - cached[0] < MARKET_CACHE_TTL_SECONDS:
    ...
    for c in ["open", "high", "low", "close"]:
        if c not in df.columns:
            raise RuntimeError(f"Market-data API missing required field: {c}")
        df[c] = pd.to_numeric(df[c], errors="coerce")

    # Volume is optional. Spot FX frequently has no real exchange volume.
    ...
    with MARKET_DATA_CACHE_LOCK:
        MARKET_DATA_CACHE[cache_key] = (now_epoch, df.copy())
    return df

def _aggregate_custom(df: pd.DataFrame, minutes: int) -> pd.DataFrame:
    """Aggregate 5-minute candles; volume is included only when available."""
    if df.empty:
        return df
    ...
    out = x.resample(f"{minutes}min", label="left", closed="left").agg(agg)
...
def fetch_candles(symbol: str, interval: str, outputsize: int = 250) -> pd.DataFrame:
    ...
    if tf in CUSTOM_INTERVAL_MINUTES:
        # 5m is the common denominator for 30/35/45/60-minute candles.
        # Request enough source bars for the requested number of custom bars.
        source_size = min(5000, max(250, int(outputsize * CUSTOM_INTERVAL_MINUTES[tf] / 5) + 60))
        base = _provider_fetch_candles(symbol, "5min", source_size)
        out = _aggregate_custom(base, CUSTOM_INTERVAL_MINUTES[tf])
        return out.tail(outputsize).reset_index(drop=True)

    return _provider_fetch_candles(symbol, tf, outputsize)



def interval_minutes(interval):
    tf = normalize_interval(interval)
    return {"1min": 1, "5min": 5, "15min": 15, "30m": 30, "35m": 35, "45m": 45, "1h": 60}.get(tf, 5)

def completed_candles(df, interval):
    """Drop the current in-progress candle when market data includes it."""
    if df is None or df.empty:
        return df
    x = df.copy()
    x["datetime"] = pd.to_datetime(x["datetime"], utc=True)
    mins = interval_minutes(interval)
    last_ts = x["datetime"].iloc[-1].to_pydatetime()
    if datetime.now(timezone.utc) < last_ts + timedelta(minutes=mins):
        return x.iloc[:-1].reset_index(drop=True)
    return x.reset_index(drop=True)

def candle_timing(interval, last_closed_timestamp=None):
    mins = interval_minutes(interval)
    now = datetime.now(timezone.utc)

    if last_closed_timestamp:
        try:
            last_ts = pd.to_datetime(last_closed_timestamp, utc=True).to_pydatetime()
            next_open = last_ts + timedelta(minutes=mins)
            while next_open <= now:
                next_open += timedelta(minutes=mins)
        except Exception:
            next_open = now + timedelta(minutes=mins)
    else:
        epoch = datetime(1970, 1, 1, tzinfo=timezone.utc)
        elapsed = int((now - epoch).total_seconds() // (mins * 60))
        next_open = epoch + timedelta(minutes=(elapsed + 1) * mins)

    wait_seconds = max(0, int((next_open - now).total_seconds()))
    window_end = next_open + timedelta(minutes=mins)
    wait_minutes = round(wait_seconds / 60, 1)
    action = "ENTER AT NEXT CANDLE OPEN" if wait_seconds <= 30 else f"WAIT {wait_minutes} MIN → NEXT CANDLE OPEN"
...
ML_CACHE_TTL_SECONDS = int(os.getenv("ML_CACHE_TTL_SECONDS", "900"))
ML_MIN_SAMPLES = int(os.getenv("ML_MIN_SAMPLES", "180"))
...
def _ml_features(df):
    ...
    body = (close - open_) / close.replace(0, np.nan)
    upper = (high - np.maximum(open_, close)) / close.replace(0, np.nan)
    lower = (np.minimum(open_, close) - low) / close.replace(0, np.nan)
    vol = ret1.rolling(20).std()

    f = pd.DataFrame({
        "ret1": ret1, "ret2": ret2, "ret3": ret3, "ret5": ret5,
        "ema_gap": (e9 - e21) / close.replace(0, np.nan),
        "rsi": rv / 100.0,
        "macd_gap": (ml - ms) / close.replace(0, np.nan),
        "atr_pct": av / close.replace(0, np.nan),
        "adx": ax / 100.0,
        "bb_pos": (close - bl) / (bu - bl).replace(0, np.nan),
        "stoch_k": sk / 100.0,
        "stoch_d": sd / 100.0,
        "range_pct": rng,
        "body_pct": body,
...
def _fit_ml_models(df):
    if not SKLEARN_OK:
        return None
    x = df.copy().reset_index(drop=True)
    feats = _ml_features(x)
    ...
    models = [
        ...
        RandomForestClassifier(
            n_estimators=220, max_depth=8, min_samples_leaf=4,
    ...
    test_pred = (avg >= 0.5).astype(int)
    oos_accuracy = float(np.mean(test_pred == y_test)) if len(y_test) else 0.5
...
def ml_ensemble_prediction(symbol, interval, df=None):
    if not SKLEARN_OK:
        return {"available": False, "probability": None, "direction": "WAIT",
                "oos_accuracy": None, "samples": 0, "model_count": 0,
                "reason": "Install scikit-learn"}

    key = (normalize_symbol(symbol), normalize_interval(interval))
    now = datetime.now(timezone.utc)
    model_pack = None

    with ML_CACHE_LOCK:
        cached = ML_CACHE.get(key)

    if cached:
        try:
            age = (now - datetime.fromisoformat(cached["trained_at"])).total_seconds()
            if age < ML_CACHE_TTL_SECONDS:
                model_pack = cached
        except Exception:
            pass
    ...
    if model_pack is None:
        if df is None:
            df = fetch_candles(symbol, interval, outputsize=500)
        model_pack = _fit_ml_models(df)
        if model_pack:
            with ML_CACHE_LOCK:
                ML_CACHE[key] = model_pack

    if not model_pack:
        return {"available": False, "probability": None, "direction": "WAIT",
                "oos_accuracy": None, "samples": 0, "model_count": 0,
                "reason": "Not enough clean historical samples"}

    x = _ml_features(df).dropna()
    if x.empty:
        return {"available": False, "probability": None, "direction": "WAIT",
                "oos_accuracy": model_pack["oos_accuracy"], "samples": model_pack["samples"],
                "model_count": len(model_pack["models"]), "reason": "No valid feature row"}

    row = x.iloc[[-1]].values
    probs = []
    for model in model_pack["models"]:
        try:
    ...
    if not probs:
        return {"available": False, "probability": None, "direction": "WAIT",
                "oos_accuracy": model_pack["oos_accuracy"], "samples": model_pack["samples"],
                "model_count": len(model_pack["models"]), "reason": "Prediction unavailable"}

    p = float(np.mean(probs))
    direction = "UP" if p >= 0.62 else "DOWN" if p <= 0.38 else "WAIT"
    return {
        "available": True,
        "probability": round(p * 100, 2),
        "direction": direction,
        "oos_accuracy": model_pack["oos_accuracy"],
        "samples": model_pack["samples"],
        "model_count": len(model_pack["models"]),
        "reason": "Ensemble: Logistic + Random Forest + Gradient Boosting",
    }
...
def make_signal(df: pd.DataFrame):
    ...
    reasons = []
    tags = []
    tag_signs = []  # (tag, +1 or -1) for each contribution, used for "model agreement" count
    ...
    def add(tag, delta, text):
        ...
        tags.append(tag)
        tag_signs.append((tag, 1 if delta > 0 else -1))
    ...
    with DB_LOCK:
        ...
        cur = con.execute("""
            ...
            (symbol, interval, timestamp, price, direction, score, confidence,
             ...
             tags_json, session, model_probability, power_quality,
            entry_wait_minutes, entry_time, window_end, timing_action, mtf_agreement)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            ...
            (s.get("ml") or {}).get("probability"), s.get("power_quality"),
            s.get("entry_wait_minutes"), s.get("entry_time"), s.get("window_end"),
            s.get("timing_action"), s.get("mtf_agreement")
        ))
        con.commit()
...
def calculate_signal(symbol, interval):
    ...
    closed = completed_candles(df, tf)
    if len(closed) < 70:
        closed = df.iloc[:-1].copy() if len(df) > 70 else df.copy()

    s = make_signal(closed)

    try:
        mtf = multi_timeframe_signal(normalize_symbol(symbol))
        s["mtf_agreement"] = mtf.get("agreement", "N/A")
        s["mtf_consensus"] = mtf.get("consensus", "WAIT")
        if mtf.get("consensus") in ("UP", "DOWN") and s["direction"] in ("UP", "DOWN"):
            if mtf["consensus"] != s["direction"]:
                s["direction"] = "WAIT"
                s["reasons"] = list(s.get("reasons", [])) + ["MTF conflict → WAIT"]
    except Exception:
        s["mtf_agreement"] = "Unavailable"
        s["mtf_consensus"] = "WAIT"

    try:
        ml = ml_ensemble_prediction(symbol, tf, df=closed)
    except Exception as e:
        ml = {"available": False, "probability": None, "direction": "WAIT",
              "oos_accuracy": None, "samples": 0, "model_count": 0, "reason": str(e)}

    s["ml"] = ml
    tech_dir = s.get("direction", "WAIT")

    if ml.get("available") and tech_dir in ("UP", "DOWN"):
        if ml["direction"] == tech_dir:
            s["power_quality"] = int(min(99, round(
                0.45 * s.get("confidence", 0) +
                0.35 * ml["probability"] +
                0.20 * (ml.get("oos_accuracy") or 50)
            )))
        elif ml["direction"] in ("UP", "DOWN"):
            s["direction"] = "WAIT"
            s["power_quality"] = min(59, int(s.get("confidence", 0) * 0.7))
            s["reasons"] = list(s.get("reasons", [])) + ["ML/technical conflict → WAIT"]
        else:
            s["power_quality"] = min(69, int(s.get("confidence", 0) * 0.8))
    else:
        s["power_quality"] = int(s.get("confidence", 0))

    timing = candle_timing(tf, s.get("timestamp"))
    s["timing"] = timing
    s["entry_wait_minutes"] = timing["wait_minutes"]
    s["entry_time"] = timing["next_open"]
    s["window_end"] = timing["window_end"]
    s["timing_action"] = timing["action"]

    s["id"] = save_signal(normalize_symbol(symbol), tf, s)
    return s
...
def multi_timeframe_signal(symbol):
    ...
    """
    results = {}
    for tf in ["1min", "5min", "15min", "30m", "35m", "45m", "1h"]:
        try:
            df = fetch_candles(symbol, tf)
            if len(df) < 60:
                results[tf] = None
...
INTERVAL_MINUTES = {"1min": 1, "5min": 5, "15min": 15, "30m": 30, "35m": 35, "45m": 45, "1h": 60}

def resolve_pending_signals():
    """
    Resolve a signal against the CLOSE of the next fully completed candle.
    For a signal made from candle T, the target is candle T+1 close.
    We wait two timeframe lengths so that the target candle is fully closed.
    """
    with DB_LOCK:
        con = db()
        pending = con.execute(
    ...
    for row in pending:
        symbol, interval = row["symbol"], normalize_interval(row["interval"])
        try:
        ...
        minutes_needed = INTERVAL_MINUTES.get(interval)
        if not minutes_needed:
            continue

        age_minutes = (datetime.now(timezone.utc) - sig_time).total_seconds() / 60
        if age_minutes < (minutes_needed * 2):
            continue

        key = (symbol, interval)
        try:
            if key not in cache:
                cache[key] = fetch_candles(symbol, interval, outputsize=250)
            df = cache[key]
        except Exception:
            continue

        # First candle AFTER the signal candle = next candle.
        target_time = sig_time + timedelta(minutes=minutes_needed)
        future = df[df["datetime"] >= target_time]
        if future.empty:
            continue

        # Only use a candle whose close should be known now.
        target = future.iloc[0]
        if target["datetime"] + timedelta(minutes=minutes_needed) > datetime.now(timezone.utc):
            continue
        ...
        if "UP" in row["direction"]:
            result = "WIN" if future_price > entry_price else "LOSS" if future_price < entry_price else "FLAT"
        else:
            result = "WIN" if future_price < entry_price else "LOSS" if future_price > entry_price else "FLAT"
...
def format_signal(symbol, interval, s):
    ...
    try:
        ...
        interval_minutes = INTERVAL_MINUTES.get(interval_norm, 5)
        last_ts = datetime.fromisoformat(s["timestamp"])
        if last_ts.tzinfo is None:
            last_ts = last_ts.replace(tzinfo=timezone.utc)
        elapsed_min = (datetime.now(timezone.utc) - last_ts).total_seconds() / 60
        remaining_min = max(0, round(interval_minutes - elapsed_min))
    except Exception:
        pass

    # --- Trend label (from EMA/MACD direction, not a forecast — describes current state) ---
    ema_bull = s["ema9"] > s["ema21"]
    ...
    # --- Momentum label (from score strength) ---
    conf = s["confidence"]
    if conf >= 70:
    ...
    # --- Model agreement (how many indicator "models" point the same way as the final call) ---
    tag_signs = s.get("tag_signs", [])
    overall_sign = 1 if s["score"] >= 0 else -1
    agree = sum(1 for _, sign in tag_signs if sign == overall_sign)
    total = len(tag_signs)

    # --- Confidence filter label (mirrors what a "signal quality" filter would show) ---
    if s["direction"] == "WAIT":
        quality_label = "⚪ NO SIGNAL (indicators disagree — sit out)"
    elif conf >= 70:
    ...
    header = (
        ...
        f"⏱️ Timeframe: {normalize_interval(interval)}\n"
        f"{arrow} Estimate: {s['direction']}\n"
        f"🎯 Score strength: {conf}%\n"
        f"📊 Trend: {trend_label}\n"
        f"⚡ Momentum: {momentum_label}\n"
        f"📈 Model agreement: {agree}/{total} indicators\n"
        f"{quality_label}\n\n"
        f"⚠️ No technology can guarantee what the next candle does — this is a "
        f"probability-style estimate from current indicators only, and it will "
        f"sometimes be wrong. It is not financial advice.\n\n"
    )

    if remaining_min is not None:
        header += (
            f"🕒 Based on the candle that closed at {last_ts.strftime('%H:%M UTC')}. "
            f"Current candle closes in ~{remaining_min} min.\n\n"
        )

    ml = s.get("ml") or {}
    ml_line = (
        f"🧠 ML ensemble: {ml.get('probability')}% {ml.get('direction')} "
        f"(OOS {ml.get('oos_accuracy')}%, {ml.get('model_count', 0)} models)\n"
        if ml.get("available") else "🧠 ML ensemble: unavailable / WAIT\n"
    )
    power_line = f"🔥 Power quality: {s.get('power_quality', conf)}/100\n"
    timing = s.get("timing") or candle_timing(interval, s.get("timestamp"))
    timing_block = (
        f"⏰ ENTRY TIMING (estimated)\n"
        f"➡️ {timing['action']}\n"
        f"🕐 Entry: {pd.to_datetime(timing['next_open'], utc=True).strftime('%H:%M:%S UTC')}\n"
        f"⌛ Duration: {timing['duration_minutes']} minutes\n\n"
    )

    return (
        header +
        ml_line + power_line + timing_block +
...
def api_power():
    return {
        "engine": "NexCandle AI V4",
        "timeframes": ["1m", "5m", "15m", "30m", "35m", "45m", "1h"],
        "ml_available": bool(SKLEARN_OK),
        "ml_models": ["LogisticRegression", "RandomForest", "HistGradientBoosting"] if SKLEARN_OK else [],
        "timing_engine": True,
        "walk_forward_backtest": True,
        "custom_candle_aggregation": True,
        "no_trade_filter": True,
        "note": "Probabilistic analysis only; no guaranteed future-candle prediction."
    }
...
def api_accuracy():
    ...
    for r in rows:
        by_timeframe.append({
            ...
            "wins": r["wins"],
            "losses": r["losses"],
            "accuracy_percent": round(r["wins"] / r["resolved"] * 100, 2) if r["resolved"] else None,
        })
    ...
    return {
        ...
        "note": "Historical hit-rate of resolved bot signals only. Accuracy needs enough resolved samples and is not a future guarantee.",
    }
...
def _direction_matches(direction, entry, target):
    if direction == "UP":
        return target > entry
    if direction == "DOWN":
        return target < entry
...
def historical_backtest(symbol, interval="5m", samples=250, min_score=50):
    """
    Genuine walk-forward, next-candle-close backtest.
    No future rows are included in the indicator window used for each prediction.
    """
    tf = normalize_interval(interval)
    source_needed = samples + 90
    df = fetch_candles(symbol, tf, outputsize=min(5000, source_needed))
    if len(df) < 80:
        raise RuntimeError("Not enough historical candles for backtest")

    start_i = max(60, len(df) - samples - 1)
    rows = []
    for i in range(start_i, len(df) - 1):
        window = df.iloc[:i+1].copy()
        try:
            s = make_signal(window)
        except Exception:
            continue
        ...
        if direction not in ("UP", "DOWN"):
            continue
        if abs(int(s["score"])) < min_score:
            continue

        entry = float(window.iloc[-1]["close"])
        target = float(df.iloc[i+1]["close"])
        outcome = "WIN" if _direction_matches(direction, entry, target) else "LOSS"
        rows.append({
            "timestamp": window.iloc[-1]["datetime"].isoformat(),
            "direction": direction,
            "score": int(s["score"]),
            "entry": entry,
            "target": target,
    ...
    return {
        "symbol": normalize_symbol(symbol),
        "interval": tf,
        "samples_tested": resolved,
        "wins": wins,
        "losses": losses,
        "accuracy_percent": accuracy,
        "min_score": min_score,
        "method": "walk-forward / next-candle-close",
        "note": "Historical simulation only; not a guarantee of future performance.",
        "trades": rows[-100:],
    }
...
def api_backtest_report(
    ...
    samples: int = Query(250, ge=50, le=1000),
    min_score: int = Query(50, ge=0, le=100),
):
    try:
        live_bt = historical_backtest(symbol, interval, samples, min_score)
    except Exception as e:
        live_bt = {"error": str(e)}
    ...
    for r in rows:
        ...
        report.append({
            ...
            "losses": r["losses"], "flats": r["flats"],
            "accuracy_percent": round(wins / resolved * 100, 2) if resolved else None,
        })

    return {
        "walk_forward": live_bt,
        "database_history": report,
        "note": "Walk-forward is a historical simulation; database accuracy is actual resolved bot signals. Neither guarantees future results.",
    }
...
async def scheduler_loop():
    """
    Periodically computes a fresh signal for every symbol in WATCHLIST and
    optionally pushes non-WAIT signals to a configured Telegram chat.
    Does nothing (idles) if WATCHLIST is empty. Signal outcome resolution
    (WIN/LOSS grading) is handled separately by resolution_loop(), which
    always runs regardless of WATCHLIST — so /accuracy stays accurate even
    if this scheduler is idle.
    """
    if not WATCHLIST:
...
RESOLUTION_INTERVAL_SECONDS = int(os.getenv("RESOLUTION_INTERVAL_SECONDS", "60"))

async def resolution_loop():
    """
    Always runs, independent of WATCHLIST or any other configuration.
    Periodically checks for PENDING signals whose timeframe has elapsed and
    grades them WIN/LOSS/FLAT against real price action. This is what keeps
    /accuracy and /backtest genuinely up to date no matter how the bot is
    ...
    while True:
        try:
            ...
            if resolved:
                print(f"Resolution loop: resolved {resolved} past signal(s).")
        except Exception as e:
            print(f"Resolution loop error: {e}")
        await asyncio.sleep(RESOLUTION_INTERVAL_SECONDS)

def start_resolution_thread():
    def runner():
        asyncio.run(resolution_loop())
    t = threading.Thread(target=runner, daemon=True)
    t.start()
            ...
            for uid in active_users:
                ...
                for item in watch:
                    ...
                    try:
                        ...
                        s = make_signal(df)
                        effective_threshold = item["custom_threshold"] or PREMIUM_ALERT_THRESHOLD
                        if abs(s["score"]) >= effective_threshold:
                            tg_app = _telegram_app_ref["app"]
                            if tg_app:
                                await tg_app.bot.send_message(
                                    chat_id=uid,
...
def build_main_menu():
    """Flat menu — all tools visible on one screen (no sub-category taps)."""
    rows = [
        ...
        [InlineKeyboardButton("⏰ Entry Timing", callback_data="m:timing"),
         InlineKeyboardButton("🔥 Best Setup", callback_data="m:best")],
...
def build_account_menu():
    rows = [
        ...
        [InlineKeyboardButton("📈 Backtest", callback_data="m:back"),
         InlineKeyboardButton("⚖️ Weights", callback_data="m:wt")],
        [InlineKeyboardButton("💎 Subscribe / Status", callback_data="m:subscribe")],
        [InlineKeyboardButton("« Back to menu", callback_data="m:main")],
    ]
    return InlineKeyboardMarkup(rows)
...
def build_persistent_keyboard():
    """A small always-visible reply keyboard so the menu is one tap away
    even after scrolling past old messages."""
...
def build_timeframe_menu(prefix, sym):
    rows = [[
        ...
        InlineKeyboardButton("35m", callback_data=f"{prefix}:{sym}:35m"),
        InlineKeyboardButton("45m", callback_data=f"{prefix}:{sym}:45m"),
...
def build_scan_timeframe_menu():
    rows = [[
        ...
        InlineKeyboardButton("30m", callback_data="scan:30m"),
        InlineKeyboardButton("35m", callback_data="scan:35m"),
        InlineKeyboardButton("45m", callback_data="scan:45m"),
        InlineKeyboardButton("1H", callback_data="scan:1h"),
    ], [InlineKeyboardButton("« Back to menu", callback_data="m:main")]]
    return InlineKeyboardMarkup(rows)
...
def build_subscribe_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ I've Paid — Enter Transaction ID", callback_data="m:paid")],
...
def build_help_text():
    return (
        ...
        "📊 *Get Signal* — pick a pair + timeframe, get a full technical read "
        "(RSI, MACD, EMA, patterns, support/resistance, etc.) with a "
        "direction estimate. This is where the actual signal shows up.\n\n"
        "🧭 *Multi-Timeframe* — checks 1m, 5m and 15m together and shows "
        "whether they agree.\n\n"
        "🔍 *Scan All Pairs* — ranks every supported pair by current signal "
        "strength.\n\n"
        "🔗 *Correlation* — shows how two pairs' recent price moves relate.\n\n"
        "📜 *History* — your last 10 signals.\n\n"
        "🎯 *Accuracy* / 📈 *Backtest* — this bot's own historical hit-rate. "
        "Shows \"No resolved signals yet\" until you've pulled a few UP/DOWN "
        "signals (not WAIT) and waited past their timeframe (e.g. 5 min for "
        "a 5m signal) so the outcome can be graded.\n\n"
        "⚖️ *Weights* — how much each indicator is currently trusted, based "
        "on this bot's own track record (needs enough resolved history first).\n\n"
        "📰 *News* — upcoming high-impact economic events (needs the bot "
        "owner to set up a free news API key; otherwise shows a short note "
        "instead of data).\n\n"
        "💎 *Subscribe* — free trial is 3 signals; after that, upgrade for "
        "unlimited access plus a personal watchlist with alerts.\n\n"
        "⚠️ Nothing here predicts the future with certainty — it's current-data "
        "analysis only."
    )
...
async def tg_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🏠 Quick-access menu enabled below — tap it anytime.",
...
async def tg_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        build_help_text(), parse_mode="Markdown", reply_markup=build_main_menu()
    )
...
async def tg_mtf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("⏳ Checking 1m / 5m / 15m / 30m / 35m / 45m / 1H...")
...
async def tg_timing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        symbol, interval = parse_command_args(context.args)
        if not symbol:
            await update.message.reply_text("Example: /timing EURUSD 45m")
            return
        if not await gate(update, context):
            return
        df = await asyncio.to_thread(fetch_candles, symbol, interval)
        closed = completed_candles(df, interval)
        last_ts = closed["datetime"].iloc[-1].isoformat() if not closed.empty else None
        t = candle_timing(interval, last_ts)
        await update.message.reply_text(
            f"⏰ ENTRY TIMING\n\n"
            f"Asset: {normalize_symbol(symbol)}\n"
            f"Timeframe: {normalize_interval(interval)}\n"
            f"➡️ {t['action']}\n"
            f"🕐 Entry: {pd.to_datetime(t['next_open'], utc=True).strftime('%H:%M:%S UTC')}\n"
            f"⌛ Duration: {t['duration_minutes']} minutes\n\n"
            "Timing is based on candle boundaries, not a guarantee of price direction."
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")

async def tg_best(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if not await gate(update, context):
            return
        symbols = [SYMBOLS[k] for k in MAJOR_SYMBOL_KEYS[:8]]
        timeframes = ["5m", "15m", "30m", "35m", "45m", "1h"]
        ranked = []
        for sym in symbols:
            for tf in timeframes:
                try:
                    df = await asyncio.to_thread(fetch_candles, sym, tf, 180)
                    closed = completed_candles(df, tf)
                    if len(closed) < 70:
                        continue
                    s = make_signal(closed)
                    if s["direction"] not in ("UP", "DOWN"):
                        continue
                    ml = await asyncio.to_thread(ml_ensemble_prediction, sym, tf, closed)
                    q = s.get("confidence", 0)
                    if ml.get("available") and ml["direction"] == s["direction"]:
                        q = min(99, int(0.55*q + 0.45*ml["probability"]))
                    elif ml.get("available") and ml["direction"] in ("UP", "DOWN"):
                        q = int(q * 0.55)
                    if q >= 60:
                        ranked.append((q, sym, tf, s, ml))
                except Exception:
                    continue

        ranked.sort(key=lambda z: z[0], reverse=True)
        if not ranked:
            await update.message.reply_text("🚫 No high-quality setup found. WAIT.")
            return

        q, sym, tf, s, ml = ranked[0]
        t = candle_timing(tf, s.get("timestamp"))
        await update.message.reply_text(
            f"🔥 BEST SETUP\n\n{sym} — {tf}\n"
            f"{'📈' if s['direction']=='UP' else '📉'} {s['direction']}\n"
            f"Quality: {q}/100\n"
            f"ML: {ml.get('probability')}% ({ml.get('direction')})\n"
            f"➡️ {t['action']}\n"
            f"🕐 Entry: {pd.to_datetime(t['next_open'], utc=True).strftime('%H:%M:%S UTC')}\n"
            f"⌛ Duration: {t['duration_minutes']} min\n\n"
            "Historical/technical analysis only; no guarantee of the next candle."
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Error: {e}")
...
async def tg_mywatch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not items:
        await update.message.reply_text(
            ...
            f"High-confidence alerts (default score ≥{PREMIUM_ALERT_THRESHOLD}) get pushed to you "
            f"automatically, checked every ~{PREMIUM_SCAN_INTERVAL_SECONDS}s. Customize per-symbol with "
            f"/setthreshold SYMBOL VALUE."
        )
        return
    lines = ["⭐ *Your Watchlist*\n"]
    for r in items:
        th = r["custom_threshold"] or PREMIUM_ALERT_THRESHOLD
        custom_tag = "" if not r["custom_threshold"] else " (custom)"
        lines.append(f"• {r['symbol']} — alert threshold: {th}%{custom_tag}")
    lines.append(
        f"\nDefault threshold is {PREMIUM_ALERT_THRESHOLD}%. Change per-symbol with "
        f"/setthreshold SYMBOL VALUE (e.g. /setthreshold GBPJPY 90)."
    )
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")

async def tg_setthreshold(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    allowed, reason, remaining = has_access(user.id, user.username)
    if reason != "subscribed":
        await update.message.reply_text(require_premium_text(), parse_mode="Markdown", reply_markup=build_subscribe_menu())
    ...
    if len(context.args) < 2:
        await update.message.reply_text(
            f"Example: /setthreshold GBPJPY 90\n\n"
            f"Sets how strong the signal score must be (as a % of max strength, "
            f"{MIN_ALERT_THRESHOLD}-{MAX_ALERT_THRESHOLD}) before you get a push alert for that symbol. "
            f"Higher = fewer but stronger alerts."
        )
        return
    symbol = context.args[0]
    try:
        ...
        return
    updated, applied = set_watchlist_threshold(user.id, symbol, value)
    if not updated:
        await update.message.reply_text(
            f"{normalize_symbol(symbol)} isn't in your watchlist yet. Add it first with /addwatch."
        )
        return
    await update.message.reply_text(
        f"✅ {normalize_symbol(symbol)} alert threshold set to {applied}%."
    )
...
async def tg_fallback_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Catches any plain text message that isn't a recognized command. If the
    user was asked to enter a transaction ID (after tapping "I've Paid"),
    this text is treated as that reference and forwarded to the bot owner.
    Otherwise, it just shows the main button menu, so nobody gets stuck not
    knowing what to type.
    """
    user = update.effective_user
    text = update.message.text or ""

    if is_awaiting_txn(user.id):
        set_awaiting_txn(user.id, False)
        row = get_or_create_user(user.id, user.username)
        if ADMIN_TELEGRAM_ID:
            try:
                ...
                pass
        await update.message.reply_text(
            "✅ Got it — your transaction ID has been sent for approval. "
            "You'll be upgraded shortly once verified.",
            reply_markup=build_subscribe_menu(),
        )
        return
...
async def safe_edit(query, text, **kwargs):
    ...
    "Message is not modified" error, which fires harmlessly whenever a
    button is tapped again on content that's already showing (e.g. tapping
    the same menu twice in a row). Any other error still propagates.
    """
    try:
    ...
    try:
        if data == "m:timing":
            await query.message.reply_text(
                "⏰ Entry Timing\n\nUse /timing EURUSD 45m for candle timing.",
                reply_markup=build_main_menu()
            )
            return
        if data == "m:best":
            await tg_best(update, context)
            return

        # --- Main menu navigation ---
        if data == "m:main":
            await safe_edit(query, MENU_INTRO, parse_mode="Markdown", reply_markup=build_main_menu())
            return
        if data == "m:help":
            await safe_edit(query, build_help_text(), parse_mode="Markdown", reply_markup=build_back_menu())
            return

        # --- Category sub-menus ---
        if data == "cat:analysis":
            await safe_edit(query, "📊 *Analysis Tools*\n\nChoose one:", parse_mode="Markdown", reply_markup=build_analysis_menu())
            return
        if data == "cat:account":
            await safe_edit(query, "📁 *My Account*\n\nChoose one:", parse_mode="Markdown", reply_markup=build_account_menu())
            return
        if data == "cat:info":
            await safe_edit(query, "ℹ️ *News & Help*\n\nChoose one:", parse_mode="Markdown", reply_markup=build_info_menu())
            return

        # --- Get Signal flow: m:sig -> sig:SYM -> sig:SYM:TF ---
        if data == "m:sig":
            await safe_edit(query, "Choose a pair:", reply_markup=build_symbol_menu("sig"))
            return
        ...
        if action == "sig" and len(parts) == 3:
            ...
            if not await gate(update, context):
                return
            await safe_edit(query, f"⏳ Analyzing {SYMBOLS.get(sym, sym)} ({tf})...")
            df = await asyncio.to_thread(fetch_candles, sym, tf)
            s = await asyncio.to_thread(calculate_signal, sym, tf)
            await context.bot.send_message(
        ...
        if data == "m:mtf":
            await safe_edit(query, "Choose a pair for 1m+5m+15m+30m+35m+45m+1H check:", reply_markup=build_symbol_menu("mtf"))
            return
        if action == "mtf" and len(parts) == 2:
            sym = parts[1]
            if not await gate(update, context):
                return
            await safe_edit(query, f"⏳ Checking {SYMBOLS.get(sym, sym)} across timeframes...")
        ...
        if data == "m:scan":
            await safe_edit(query, "Choose a timeframe to scan all pairs:", reply_markup=build_scan_timeframe_menu())
            return
        if action == "scan" and len(parts) == 2:
            tf = parts[1]
            if not await gate(update, context):
                return
            await safe_edit(query, f"⏳ Scanning all pairs on {tf}...")
        ...
        if data == "m:corr1":
            await safe_edit(query, "Choose the FIRST pair:", reply_markup=build_symbol_menu("corr1"))
            return
        ...
        if action == "corr2" and len(parts) == 3:
            ...
            if not await gate(update, context):
                return
            await safe_edit(query, f"⏳ Computing correlation {sym_a} vs {sym_b}...")
        ...
        if data == "m:hist":
            await safe_edit(query, "⏳ Loading history...")
            await tg_history_content(context.bot, query.message.chat_id)
            return
        if data == "m:acc":
            await safe_edit(query, "⏳ Loading accuracy...")
            ...
            return
        if data == "m:back":
            await safe_edit(query, "⏳ Loading backtest report...")
            back_data = api_backtest_report()
            ...
            return
        if data == "m:wt":
            await safe_edit(query, "⏳ Loading adaptive weights...")
            weights = get_adaptive_weights()
            if not weights:
            ...
            return
        if data == "m:news":
            await safe_edit(query, "⏳ Loading news...")
        ...
        if data == "m:paid":
            user = update.effective_user
            set_awaiting_txn(user.id, True)
            await safe_edit(query, 
                "✍️ Please type or paste your *transaction ID / reference number* now "
                "(from your UPI or BOTIM Pay app).\n\n"
                "It'll be sent straight to the bot owner for approval, along with a "
                "payment screenshot if you have one.",
                parse_mode="Markdown",
                reply_markup=build_back_menu(),
            )
            return
        ...
        if data == "m:status":
            ...
            row = get_or_create_user(user.id)
            txt = build_status_text(reason, remaining, row)
            await safe_edit(query, txt, parse_mode="Markdown", reply_markup=build_subscribe_menu())
            return
...
async def run_telegram():
    ...
    tg_app.add_handler(CommandHandler("timing", tg_timing))
    tg_app.add_handler(CommandHandler("best", tg_best))
    ...
    tg_app.add_handler(CommandHandler("setthreshold", tg_setthreshold))
