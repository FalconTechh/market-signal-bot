...
INDIA_UPI = (os.getenv("INDIA_UPI") or os.getenv("UPI_ID") or os.getenv("PAYMENT_UPI") or "").strip()
UAE_BOTIM = (os.getenv("UAE_BOTIM") or os.getenv("BOTIM_NUMBER") or os.getenv("PAYMENT_BOTIM") or "").strip()
...
def make_signal(pair, tf):
    ...
    score = int(min(100, round(s * 0.60 + ms * 0.40)))
    ...
    if final != "WAIT":
        ...
        if final == "CALL":
            stop = e - risk; target = e + reward
        else:
            stop = e + risk; target = e - reward
        entry = e; rr = reward / risk
    return {"pair":pair,"tf":tf,"direction":final,"score":score,"why":why,"mtf":m,
            "wait":max(0,next_candle-int(time.time())),"entry":entry,"stop":stop,"target":target,"rr":rr,
            "candle":x.index[-1].isoformat(),"provider":meta["provider"],"data_age":meta["age"],
            "fresh":meta["fresh"],"next_candle":next_candle,"entry_start":entry_start,"entry_end":entry_end}

def fmt_signal(s, detailed=True):
    lines=["⚡ <b>NexCandle AI — ADVANCED SIGNAL</b>",f"💱 <b>{s['pair']}</b> • ⏱ <b>{s['tf']}</b>","",
           f"📡 Data: <b>{s['provider']}</b> • Age: <b>{fmt_duration(s['data_age'])}</b>"]
    if s["direction"]=="WAIT":
        lines += ["","🟡 <b>NO TRADE / WAIT</b>",f"🎯 Setup quality: <b>{s['score']}/100</b>",
                  "Reason: local + higher-timeframe confirmation is not strong enough.","⛔ Do not force or chase an entry.","",fmt_signal_timing(s)]
    else:
        lines += ["",f"📌 <b>{s['direction']}</b>",f"🎯 Setup quality: <b>{s['score']}/100</b>","",
                  "🕐 <b>ENTRY TIMING</b>",fmt_signal_timing(s),"",
                  f"📍 Reference price: <code>{s['entry']:.6f}</code>",f"🛑 SL reference: <code>{s['stop']:.6f}</code>",
                  f"🎯 TP reference: <code>{s['target']:.6f}</code>",f"⚖️ Risk/Reward: <b>1:{s['rr']:.2f}</b>","",
                  "✅ Enter only after the setup remains confirmed inside the entry window.",
                  "🚫 If the window expires or confirmation changes, WAIT for a new signal."]
    if detailed:
        lines += ["","🧭 <b>MTF CONFIRMATION</b>"]
        for tf,(d,sc) in s["mtf"].items(): lines.append(f"{tf}: {d} ({sc})")
        lines += ["","🧠 <b>ANALYSIS</b>","• "+"\n• ".join(s["why"][:6]),"",
                   "⚠️ Setup score is not a guaranteed probability or profit forecast."]
    return "\n".join(lines)
...
async def premium_screen(q,uid):
    ...
    text="💎 <b>NEXCANDLE PREMIUM</b>\n\nAdvanced scanner • Backtest • Auto alerts • Performance tracking"+price+"\n\nChoose a payment method. After payment, send the transaction/reference ID or proof screenshot. Admin approval activates Premium."
    rows=[]
    ...
    if not rows:
        text += "\n\n⚠️ Payment methods are not configured on the server yet. Set UPI_ID/PAYMENT_UPI or BOTIM_NUMBER/PAYMENT_BOTIM in Render Environment."
