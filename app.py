...
def telegram_webhook():
    ...
    try:
        ...
        # Dispatch the update directly on the Telegram asyncio loop.
        # This avoids webhook updates getting stuck in the application's
        # internal queue when Render/Flask and PTB are running on separate
        # threads. The existing handlers and bot logic remain unchanged.
        fut = asyncio.run_coroutine_threadsafe(
            TELEGRAM_APP.process_update(update),
            TELEGRAM_LOOP,
        )
        fut.add_done_callback(
            lambda f: db_log("warning", "telegram_update", str(f.exception()))
