...
BUILD_ID = "V35-BALANCED-CANDLE-10-PAIRS-1M-30M-1H-2026-08-28"
...
SIGNAL_CACHE_TTL_BY_TF = {"1m": 8, "30m": SIGNAL_CACHE_SECONDS, "1h": SIGNAL_CACHE_SECONDS}
...
def refresh_forex_catalog(force=False):
    """Return the fixed public 10-pair universe.
...
def make_v29_signal(pair, tf):
    if tf not in V29_TFS:
        raise RuntimeError("V29 supports only 1m, 30m and 1h")
    ...
    if len(cs)<60:
        raise RuntimeError(f"Insufficient completed higher-timeframe candles: {len(cs)}")
    ...
    return {
        ...
        "model":"V35 1M/30M/1H Candle Fusion",
