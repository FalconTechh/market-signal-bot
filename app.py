...
def analyze_candles(candles, timeframe=None):
    """
    Next-candle direction classifier.

    IMPORTANT:
    - Only completed candles are used.
    - The currently forming candle is excluded.
    - Uses trend, momentum, RSI, candle structure, breakout/range context
      and volatility normalization.
    - Weak confluence returns WAIT instead of fabricating UP/DOWN.
    - Confidence is a model score, NOT a guarantee of future direction.
    """
    cs=[]
    for c in candles or []:
        if c.get("isOpen") is True or c.get("open_candle") is True:
            continue
        o=_nc_num(c.get("open")); h=_nc_num(c.get("high"))
        l=_nc_num(c.get("low")); cl=_nc_num(c.get("close"))
    ...
    opens=[_nc_num(c.get("open")) for c in cs]
    highs=[_nc_num(c.get("high")) for c in cs]
    lows=[_nc_num(c.get("low")) for c in cs]
    closes=[_nc_num(c.get("close")) for c in cs]
    ...
    # Trend / momentum.
    e9=_nc_ema(closes[-40:],9)
    e21=_nc_ema(closes[-60:],21)
    trend=max(-1.0,min(1.0,(e9-e21)/(atr*2.0 if atr else 1.0)))

    r3=(last_c/closes[-4]-1.0) if closes[-4] else 0.0
    r6=(last_c/closes[-7]-1.0) if closes[-7] else 0.0
    momentum=max(-1.0,min(1.0,(0.65*r3+0.35*r6)*500.0))

    rsi=max(-1.0,min(1.0,(_nc_rsi(closes,14)-50.0)/18.0))

    # Candle body/wick structure.
    upper=(last_h-max(last_o,last_c))/candle_range
    lower=(min(last_o,last_c)-last_l)/candle_range
    structure=max(-1.0,min(1.0,(body/candle_range)*1.8))
    if body > 0 and lower > upper*1.25:
        structure=min(1.0,structure+0.12)
    if body < 0 and upper > lower*1.25:
        structure=max(-1.0,structure-0.12)
    ...
    if last_c > prev_high:
        breakout=0.75
    elif last_c < prev_low:
        breakout=-0.75
    else:
        midpoint=(prev_high+prev_low)/2.0
        breakout=max(-1.0,min(1.0,(last_c-midpoint)/
                               (max(prev_high-prev_low,1e-12)*0.55)))

    vol_ratio=(candle_range/atr) if atr else 1.0
    volatility_weight=1.0 if 0.55 <= vol_ratio <= 2.2 else 0.72

    score=(0.28*trend + 0.22*momentum + 0.15*rsi +
           0.18*structure + 0.17*breakout) * volatility_weight

    direction="UP" if score > 0 else "DOWN"
    agreement=sum(
        (x > 0) == (score > 0)
        for x in (trend,momentum,rsi,structure,breakout)
    )

    # Don't force a trade direction when evidence is weak.
    if abs(score) < 0.08 or agreement < 3:
        return {
            "signal":"WAIT","direction":"WAIT","confidence":0,
            "score":round(score,4),"reason":"weak_confluence",
            "components":{
                "trend":round(trend,3),
    ...
    confidence=min(95,max(51,round(50+abs(score)*45+(agreement-2)*2)))
    return {
        "signal":direction,
        "direction":direction,
        "confidence":confidence,
        "score":round(score,4),
        "reason":"multi_factor_next_candle",
        "components":{
            "trend":round(trend,3),
            "momentum":round(momentum,3),
            "rsi":round(rsi,3),
