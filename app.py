...
BUILD_ID = "V30-FINAL-ALL-FOREX-2026-08-26"

import numpy as np
import pandas as pd
import requests

# V30 DEFINITIVE COMPLETED-CANDLE HELPER
...
def refresh_forex_catalog(force=False):
    ...
    with PAIR_CATALOG_LOCK:
        if not force and now - PAIR_CATALOG_CACHE["at"] < PAIR_CATALOG_TTL:
            return list(PAIR_CATALOG_CACHE["pairs"])
    pairs=[]
    try:
        r=S.get("https://biquote.io/api/symbols", params={"type":"Forex","quotedWithinDays":7}, timeout=5)
        data=r.json() if r.status_code==200 else []
        if isinstance(data,list):
            for item in data:
                sym=str(item.get("name") or "").upper() if isinstance(item,dict) else ""
                if len(sym)==6 and sym.isalpha():
                    pairs.append(f"{sym[:3]}/{sym[3:]}")
    except Exception:
        pass
    merged=list(dict.fromkeys(pairs + list(PAIRS.keys())))
    with PAIR_CATALOG_LOCK:
        for pair in merged:
            PAIRS.setdefault(pair, _dynamic_pair_meta(pair))
        PAIR_CATALOG_CACHE["pairs"]=merged
        PAIR_CATALOG_CACHE["at"]=now
    return merged
...
def forex_pairs_api():
    ...
    return jsonify({"ok":True,"count":len(pairs),"pairs":pairs,"source":"biquote-forex-catalogue"})
...
def _co_num(x, d=0.0):
    try:
        ...
        return d
# Backward-compatible alias: some deployed V30 builds referenced the
# completed-candle helper with a typo. Keep both names valid so that one
# bad reference cannot turn a valid market-data response into HTTP 500.
