...
def candles(pair, tf, limit=None):
    ...
    with fetch_lock:
        ...
        # Public V30 lane: 30m and 1h use native higher-timeframe candles first.
        # This avoids a stale/short 5m feed preventing an otherwise valid 30m/1h
        # signal. A 5m-derived fallback remains available only if native feeds fail.
        if tf in {"30m", "1h"}:
            direct_higher = [(n, f) for n, f in providers
                             if n not in {"sifting", "oanda"}]
            # Prefer the no-key/native OHLC feed, then configured providers.
            pref = {"biquote": 0, "yahoo2": 1, "twelvedata": 2,
                    "finnhub": 3, "alphavantage": 4, "dukascopy": 5, "yahoo": 6}
            direct_higher.sort(key=lambda item: pref.get(item[0], 99))
            for name, fn in direct_higher:
