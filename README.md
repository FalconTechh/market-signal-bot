...
## V9 market-feed fix
- SiftingIO is opt-in and disabled by default to prevent exhausted free-plan quota loops.
- Sifting websocket is disabled by default.
- Provider order starts with public fallbacks, then credentialed providers, then Yahoo.
- Rate/quota errors trigger provider cooldown instead of retry storms.
- User-facing signal UI never exposes provider/API errors.
