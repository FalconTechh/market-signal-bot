...
PROVIDER_ORDER = [x.strip().lower() for x in
                  (os.getenv("DATA_PROVIDER_ORDER") or
                   "sifting,oanda,twelvedata,finnhub,alphavantage,yahoo2,yahoo").split(",")
                  if x.strip()]
...
def get_directional_signal(pair, tf):
    ...
    try:
        ...
        if direction not in {"CALL","PUT"}:
            # A strong fresh local model can still provide a directional vote
            # when MTF confirmation is incomplete, but only above a high bar.
            x=candles(pair,tf)
            meta=market_meta(pair,tf,x)
            if not meta["fresh"]:
                raise RuntimeError("stale market data")
            direction,score,why,x=analyse(x)
            if direction not in {"CALL","PUT"} or score < max(MIN_SIGNAL_SCORE, 80):
                raise RuntimeError("No high-confidence directional setup")
            s=dict(s)
            s.update({"direction":direction,"score":int(score),"why":why,
                      "provider":meta["provider"],"data_age":meta["age"],
                      "entry":float(x.close.iloc[-1]),"target_candle":"NEXT_CANDLE"})
        with LOCK:
...
def health_data():
    ...
    providers = {}
    configured = {
        "sifting": bool(SIFTING_API_KEY), "oanda": bool(OANDA_API_TOKEN),
...
def health():
    ...
    return {"status": "ok", "version": "4.3", "bot_mode": BOT_MODE, "providers": provider,
...
def _provider_failed(name, exc):
    ...
    if any(k in lower for k in ("429", "too many", "rate limit", "rate-limit")):
...
def candles(pair, tf, limit=None):
    ...
    def cached_frame():
        with LOCK:
            ...
            if cached and time.time() - cached[0] <= _cache_ttl(tf) and len(cached[1]) >= min(limit, 80):
    ...
    with fetch_lock:
        ...
        for name, fn in providers:
            ...
            try:
                ...
                # Never analyse an in-progress candle.
        ...
        detail = "; ".join(errors[-4:]) if errors else "no configured market-data provider"
...
def analyse(x):
    ...
    factors.update({"bull":round(bull,2),"bear":round(bear,2),"gap":round(gap,2),"rsi":float(a.rsi),"adx":float(a.adx),"macd_hist":float(a.mh),"atr":float(a.atr),"atr_pct":atr_pct,"stoch_k":float(a.stoch_k),"stoch_d":float(a.stoch_d),"cci":float(a.cci),"ema_slope":float(a.ema_slope),"bb_width":float(a.bb_width),
...
def mtf(pair, entry_tf="5m"):
    ...
    denom=sum(weights[:available]) or 1
    ...
    try:
        ...
        if d.startswith("run:"):
            ...
            try:
                ...
                providers = " • ".join(provider_status()) or "No market-data provider configured"
                return await q.message.reply_text(
                    "⚠️ <b>LIVE MARKET DATA UNAVAILABLE</b>\n\n"
                    "The bot is online, but a fresh validated market feed is not available right now.\n\n"
                    f"📡 {html.escape(providers)}\n\n"
                    "Please configure/restore a working market-data provider and retry.",
                    parse_mode=ParseMode.HTML,
                    reply_markup=back_menu()
                )
            if s["direction"] not in {"CALL","PUT"}:
                return await q.message.reply_text(
                    "🟡 <b>NO CONFIRMED NEXT-CANDLE SIGNAL</b>\n\n"
                    "Fresh market data is available, but the models are not sufficiently aligned. "
                    "No forced UP/DOWN signal is generated.",
                    parse_mode=ParseMode.HTML,
                    reply_markup=back_menu()
                )
