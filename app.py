...
def fx_market_status():
    n=datetime.now(_NY); wd=n.weekday(); t=n.time()
    op=datetime.strptime("17:05","%H:%M").time()
    cl=datetime.strptime("16:59","%H:%M").time()
    if wd>=5 or (wd==4 and t>=cl): return False,n
    if t>=op or t<cl: return True,n
    return False,n
def fx_next_open():
    n=datetime.now(_NY); days=(6-n.weekday())%7
    if days==0 and n.time()>=datetime.strptime("17:05","%H:%M").time(): days=7
    d=n+timedelta(days=days)
    return d.replace(hour=17,minute=5,second=0,microsecond=0)
