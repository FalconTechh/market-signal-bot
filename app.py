...
# ============================================================
# V41 MOBILE ACCESS LOCK
# Server-side mobile detection: mobile browsers are denied before
# the engine UI/API can be used. Desktop access remains unchanged.
# The Telegram webhook is explicitly exempt so bot operation is safe.
# ============================================================
_MOBILE_UA_MARKERS = (
    "android", "iphone", "ipad", "ipod", "mobile",
    "windows phone", "opera mini", "opera mobi", "blackberry",
    "webos", "iemobile", "kindle", "silk/"
)

def _is_mobile_request():
    ua = (request.headers.get("User-Agent") or "").lower()
    # Explicitly treat common mobile/tablet browser signatures as mobile.
    return any(token in ua for token in _MOBILE_UA_MARKERS)

MOBILE_TERMINATED_HTML = r"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Account Terminated — NexCandle AI</title>
<style>
html,body{margin:0;min-height:100%;background:#020203;color:#fff;font-family:Arial,sans-serif}
body{display:grid;place-items:center;overflow:hidden}
.wrap{width:min(680px,88vw);padding:42px 24px;text-align:center;border:1px solid #5d0017;
border-radius:28px;background:radial-gradient(circle at 50% 38%,#3a0612 0,#090406 42%,#020203 76%);
box-shadow:0 0 70px rgba(255,0,45,.18),inset 0 0 60px rgba(255,0,45,.06)}
.logo{font-size:14px;letter-spacing:5px;color:#ff365f;margin-bottom:30px}
.icon{font-size:58px;filter:drop-shadow(0 0 20px #ff1744)}
h1{font-size:clamp(28px,7vw,54px);margin:16px 0 10px;letter-spacing:2px}
h2{font-size:13px;letter-spacing:4px;color:#ff315d;margin:0 0 25px}
p{color:#8e9299;line-height:1.7;font-size:14px}
.badge{display:inline-block;margin-top:16px;padding:10px 16px;border:1px solid #6e1024;
border-radius:999px;color:#ff315d;font-size:11px;letter-spacing:2px}
</style>
</head>
<body>
<div class="wrap">
  <div class="logo">NEXCANDLE AI PRO</div>
  <div class="icon">⛔</div>
  <h1>YOUR ACCOUNT IS TERMINATED</h1>
  <h2>MOBILE ACCESS BLOCKED</h2>
  <p>This service is not available from mobile devices.<br>
     Please use an authorized desktop browser to access the platform.</p>
...
@web.before_request
def _v41_mobile_access_lock():
    # Keep Telegram's server-to-server webhook and basic health checks working.
    if request.path.startswith("/telegram/webhook") or request.path in ("/health", "/"):
        # "/" is handled below so mobile users get the termination screen.
        if request.path == "/":
            pass
        else:
            return None
    ...
    if _is_mobile_request():
        # Do not expose API data or engine endpoints to mobile clients.
        return Response(MOBILE_TERMINATED_HTML, status=403, mimetype="text/html")
    return None
  ...
  <section class="card">
   <div class="controls">
    <select id="pair" class="select">
     ...
     <option>USD/CAD</option><option>USD/CHF</option><option>NZD/USD</option><option>EUR/JPY</option><option>GBP/JPY</option>
    </select>
    <select id="tf" class="select"><option>1m</option><option selected>5m</option><option>15m</option><option>30m</option><option>45m</option><option>1h</option><option>4h</option></select>
...
let lastDirection=null, busy=false;

function paint(d){
 const el=document.getElementById('result');
 el.className='result '+(d==='UP'?'up':d==='DOWN'?'down':'wait');
...
async function getSignal(p){
 ...
 try{
  const r=await fetch('/api/premium-signal?pair='+encodeURIComponent(p)+'&tf='+encodeURIComponent(document.getElementById('tf').value),{cache:'no-store',signal:ctl.signal});
  if(!r.ok) throw new Error('data');
  const j=await r.json();
  if(!j.ok || !j.signal || !['UP','DOWN','WAIT'].includes(j.signal.display_direction)) throw new Error('signal');
...
async function scan(){
 ...
 hint.textContent='3D ENGINE ACTIVE • ANALYSING MARKET STRUCTURE';
 try{
  const d=await getSignal(document.getElementById('pair').value);
  paint(d);
  hint.textContent='NEXT-CANDLE DIRECTION • LIVE VALIDATED DATA';
 }catch(e){
  // Never expose API/provider errors in the premium UI.
  // Keep the last valid UP/DOWN result visible rather than showing ERR/WAIT.
  hint.textContent=lastDirection ? 'NEXT-CANDLE ENGINE • LAST VALID DIRECTION' : 'NEXT-CANDLE ENGINE • ANALYSING';
...
async function fullScan(){
 ...
 busy=true;
 const hint=document.getElementById('hint');
 hint.textContent='FULL POWER SCAN • SEQUENTIAL VALIDATION ACTIVE';
 try{
  ...
  for(let i=0;i<pairs.length;i++){
   ...
   try{
    ...
    b.textContent=d;b.className='badge '+(d==='UP'?'up':'down');
   }catch(e){}
   if(i<pairs.length-1) await new Promise(r=>setTimeout(r,1200));
  }
  hint.textContent='FULL POWER SCAN • VALIDATED DATA';
 }finally{busy=false;}
}
...
def market_status_api():
    market=(request.args.get("market","forex") or "forex").strip().lower()
    if market=="crypto":
        return jsonify({
            "ok": True, "open": True, "market": "crypto",
            "message": "CRYPTO MARKET OPEN",
            "timezone": "UTC",
    ...
    opened,n=fx_market_status()
    payload={
        "ok": True, "open": bool(opened), "market": "forex",
...
def forex_pairs_api():
    pairs = refresh_forex_catalog()
    return jsonify({"ok": True, "count": len(PUBLIC_PAIRS), "pairs": PUBLIC_PAIRS, "build": BUILD_ID})

@web.get("/api/crypto-pairs")
def crypto_pairs_api():
    return jsonify({"ok":True,"count":len(CRYPTO_PAIRS),"pairs":CRYPTO_PAIRS,
                    "timeframes":["1m","5m","1h"],"source":"BINANCE",
                    "build":BUILD_ID})
...
def premium_signal_api():
    ...
    if market=="crypto":
        if pair not in set(CRYPTO_PAIRS):
            return jsonify({"ok":False,"error":"Unsupported crypto pair"}),400
        try:
            s=get_crypto_signal(pair,tf)
            public_signal={
                "market":"crypto","pair":pair,"tf":tf,
                "display_direction":s["display_direction"],
                "display_score":s["display_score"],
                "confidence":s["confidence"],
                "confirmation":s["confirmation"],
                "target_candle":"NEXT_CANDLE",
                "provider":"BINANCE",
                "real_candles":s["real_candles"],
                "walk_forward_accuracy":s["walk_forward_accuracy"],
                "walk_forward_samples":s["walk_forward_samples"],
                "regime":s["regime"],
            }
            return jsonify({"ok":True,"build":BUILD_ID,"signal":public_signal})
        except Exception as exc:
            trace_id=f"CRYPTO-{pair.replace('/','')}-{tf}-{int(time.time())}"
            db_log("warning","crypto-api",f"{trace_id} {pair}/{tf}: {type(exc).__name__}: {str(exc)[:300]}")
            return jsonify({"ok":False,"error":"Binance market data temporarily unavailable","trace_id":trace_id}),503

    if market!="forex":
        return jsonify({"ok":False,"error":"Unsupported market"}),400
    market_open,_now=fx_market_status()
    if not market_open:
        return jsonify({"ok":True,"market_open":False,"market":"forex","signal":None,
                        "message":"FOREX MARKET CLOSED","next_open":fx_next_open().isoformat()})
    allowed_pairs=set(PUBLIC_PAIRS)
    if pair not in allowed_pairs:
        return jsonify({"ok":False,"error":"Unsupported forex pair"}),400
    try:
        s=make_v29_signal(pair,tf)
        direction=s.get("direction")
        prediction=s.get("prediction_direction")
        display=direction if direction in {"CALL","PUT"} else prediction
        if display in {"CALL","PUT"}:
            s["display_direction"]="UP" if display=="CALL" else "DOWN"
            s["display_score"]=int(np.clip(s.get("prediction_score",s.get("score",50)),1,99))
        else:
            s["display_direction"]="WAIT"; s["display_score"]=0
        s["tradeable"]=bool(direction in {"CALL","PUT"})
        s["confirmation"]="CONFIRMED" if s["tradeable"] else "WAIT"
        public_signal={
            "market":"forex","pair":pair,"tf":tf,
            "display_direction":s.get("display_direction"),
            "display_score":s.get("display_score"),
            "confidence":s.get("confidence",s.get("display_score")),
            "confirmation":s.get("confirmation","CANDLE CONFIRMED"),
            "target_candle":"NEXT_CANDLE","candle_only":True,
        }
        return jsonify({"ok":True,"build":BUILD_ID,"signal":public_signal})
    except Exception as exc:
        trace_id=f"{pair.replace('/','')}-{tf}-{int(time.time())}"
        db_log("warning","premium-api",f"{trace_id} {pair}/{tf}: {type(exc).__name__}: {str(exc)[:300]}")
        return jsonify({"ok":False,"error":"Market data temporarily unavailable","trace_id":trace_id}),503
...
def health():
    ...
    return {"status": "ok", "version": BUILD_ID, "bot_mode": BOT_MODE, "providers": provider,
...
def _validate_candle_cadence(x, tf, min_points=3):
...
def candles(pair, tf, limit=None):
    ...
    limit = min(20, max(1, int(limit or 20)))
    key = (pair, tf)

    def cached_frame():
        with LOCK:
            cached = MARKET_CACHE.get(key)
            if cached and time.time() - cached[0] <= _cache_ttl(tf) and len(cached[1]) >= min(limit, 20):
    ...
    with fetch_lock:
        ...
        def validate_and_store(name, x):
            x = _validate_candle_cadence(x, tf, 12)
            interval = TF_SECONDS[tf]
            last_epoch = int(x.index[-1].timestamp())
            # Always remove an unfinished current candle, even when the
            # provider returned only a small recent window. Never analyze an open bar.
            if int(time.time()) - last_epoch < interval:
                x = x.iloc[:-1]
            if len(x) < 12:
                raise RuntimeError("not enough validated real candles")
            stored = x.copy()
            with LOCK:
        ...
        if tf in {"__disabled_higher_tf__"}:
        ...
        # For 1m, prefer the direct candle feed first; do not let a faster
        # but potentially delayed Yahoo edge win the race over the primary 1m feed.
        public_names = {"biquote"} if tf == "1m" else {"biquote", "yahoo2"}
        public = [(n, f) for n, f in providers if n in public_names]
...
# ================= STABLE CANDLE ENGINE ENTRYPOINT =================
# Keep this public symbol defined before every higher-level engine can call it.
# A previous deployed build raised NameError: analyze_candles is not defined.
# The implementation is resolved at request time after the module has loaded.
def analyze_candles(candles, timeframe=None):
    impl = globals().get("_analyze_candles_impl")
    if impl is None:
        # Defensive fallback: never raise NameError for a fresh OHLC payload.
        rows = []
        for c in candles or []:
            try:
        ...
        if len(rows) < 3:
            return {"signal":"WAIT","direction":"WAIT","confidence":0,"score":0,"reason":"insufficient_completed_candles"}
        ups=sum(1 for o,h,l,cl in rows[-8:] if cl>o)
        downs=sum(1 for o,h,l,cl in rows[-8:] if cl<o)
        direction="UP" if ups>=downs else "DOWN"
        return {"signal":direction,"direction":direction,"confidence":58,"score":0.16 if direction=="UP" else -0.16,"reason":"defensive_candle_fallback","candle_only":True}
    return impl(candles, timeframe)

# ================= V35 1M/30M/1H CANDLE FUSION ENGINE =================
# This is intentionally restricted to completed 1m/30m/1h candles.
...
def make_v29_signal(pair, tf):
    if tf not in V36_TFS:
        raise RuntimeError("V37 supports only 1m, 5m and 1h")
    x=candles(pair, tf, 20)
    ...
    cs=completed_candles_v30(cs)
    if len(cs)<12:
        raise RuntimeError(f"Insufficient completed real candles: {len(cs)}")
    ...
    if direction not in {"UP","DOWN"}:
        # Do not force a bullish tie-break.  If the completed-candle evidence
        # is genuinely neutral/conflicted, expose WAIT rather than inventing
        # an UP prediction.
        return {
            "pair":pair,"tf":tf,"direction":"WAIT","prediction_direction":"WAIT",
            "display_direction":"WAIT","display_score":0,"confidence":0,
            "score":0,"reason":"neutral_or_conflicting_completed_candles",
            "tradeable":False,"data_age":meta["age"],"fresh":meta["fresh"],
            "candle":x.index[-1].isoformat(),"walk_forward":None,"wf_samples":0
        }
    pred="CALL" if direction=="UP" else "PUT"
    wf, samples=_v29_walk_forward(cs, min_train=70, max_eval=90)
    # 1m is inherently noisier than 30m/1h. Require stronger recent candle
    # agreement before publishing a high-confidence 1m forecast. The score is
    # still a model score, never a guaranteed probability.
    if tf == "1m" and samples >= 20 and wf < 0.48:
        base["confidence"] = min(float(base.get("confidence", 0)), 64.0)
    ...
    confidence=max(50.0,min(96.0,float(base["confidence"])+cal_bonus))
    ...
    return {
        ...
        "model":"V37 15-Pair 20-Real-Candle Fusion",
        "walk_forward_accuracy":round(wf*100,1),"walk_forward_samples":samples,
...
def make_signal(pair, tf):
    ...
    with LOCK:
        ...
        cache_ttl = SIGNAL_CACHE_TTL_BY_TF.get(tf, SIGNAL_CACHE_SECONDS)
...
def _analyze_candles_impl(candles, timeframe=None):
    """V28 strict candle-only next-candle classifier implementation."""
    cs=completed_candles_v30(candles)
    if len(cs)<12:
    ...
    # Never resolve a neutral score to UP.  The previous implementation could
    # turn a zero/near-zero candle score into a bullish signal through a
    # downstream tie-break.  Neutral structure must remain WAIT.
    ...
    target=1 if score>0 else -1 if score<0 else 0
    agreement=(sum(v==target for v in active)/len(active)) if (active and target) else 0.0
    ...
    # Strict release gate.
    if score == 0 or abs(score)<0.14 or agreement<0.62 or confidence<58:
        return {"signal":"WAIT","direction":"WAIT","confidence":0,
...
SUPPORTED_PAIRS = list(PUBLIC_PAIRS)
# =============================================================
