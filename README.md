...
\n## v4.2.1 bot data resilience
- Telegram UI/handlers and signal logic are unchanged.
- Provider requests fail faster to prevent long stalls when a feed times out.
- When all providers have a temporary outage, the engine may use a short-lived validated in-memory candle cache; it never creates random/fake prices.
- If a fresh SiftingIO websocket tick exists, it updates the cached last candle before analysis.
- At least one valid market-data provider/API key is still required for fresh data after a restart.
