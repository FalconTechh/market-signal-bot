...
# ============================================================
# NexCandle AI v3.0 Power AI - production-oriented Telegram signal bot
# ============================================================
...
PROVIDER_MIN_INTERVAL_SECONDS = max(0.0, float(os.getenv("PROVIDER_MIN_INTERVAL_SECONDS", "0.25")))
...
@web.get("/")
def home():
    return {"service": "NexCandle AI", "version": "3.0 Power AI", "status": "online"}
...
def health():
    ...
    return {"status": "ok", "version": "3.0", "providers": provider,
...
def _provider_failed(name, exc):
    msg = str(exc)
    lower = msg.lower()
    # Back off on rate limits, timeouts and service-unavailable responses.
    if any(k in lower for k in ("429", "too many", "timeout", "timed out", "503", "502", "temporarily unavailable", "service unavailable")):
        with LOCK:
            PROVIDER_COOLDOWN[name] = time.time() + PROVIDER_COOLDOWN_SECONDS
    db_log("warning", "market", f"{name}: {msg}")
...
def candles(pair, tf, limit=350):
    ...
    for name, fn in providers:
        if not _provider_ready(name):
            errors.append(f"{name}: cooldown/rate-limit protection active")
            continue
        try:
            x = fn(pair, tf, limit)
            if len(x) < 80:
...
def provider_status():
    now = time.time()
    out = []
    for name, key in (("Finnhub", FINNHUB_API_KEY), ("TwelveData", TWELVEDATA_API_KEY)):
        if key:
            with LOCK:
                cooldown = max(0, int(PROVIDER_COOLDOWN.get(name.lower(), 0) - now))
            out.append(f"{name} (cooldown {cooldown}s)" if cooldown else f"{name} (ready)")
    with LOCK:
        cooldown = max(0, int(PROVIDER_COOLDOWN.get("yahoo", 0) - now))
    out.append(f"Yahoo fallback (cooldown {cooldown}s)" if cooldown else "Yahoo fallback (ready)")
    return out
