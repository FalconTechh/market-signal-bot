# NexCandle AI v4.4 — Hyper-Reliable Next-Candle Engine
...
## v4.4 Telegram webhook fix
- Uses Render's live external hostname first; avoids stale webhook URLs.
- Processes Telegram updates directly on the bot event loop instead of relying on a queue-only bridge.
- Verifies the webhook after startup and logs pending updates/last webhook error.
- OANDA is not required or used by the default provider order.

After deploy, open `/health/telegram` on the current Render service. It should show `webhook_url_set: true` and `status: ok`.
