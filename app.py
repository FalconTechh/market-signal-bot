...
# Render should use Telegram webhook by default. Polling on Render can create
# Telegram 409 Conflict when an older instance is still alive during deploys.
BOT_MODE = (os.getenv("BOT_MODE") or ("webhook" if RENDER_EXTERNAL_HOSTNAME else "polling")).strip().lower()
...
@web.get("/")
def home():
    return {"service": "NexCandle AI", "version": "3.3 Broker-Grade Market Data", "status": "online"}
...
def health():
    ...
    return {"status": "ok", "version": "3.3", "bot_mode": BOT_MODE, "providers": provider,
...
async def telegram_loop_runner(app):
    ...
    if BOT_MODE == "webhook":
        ...
        await app.bot.delete_webhook(drop_pending_updates=True)
        db_log("info", "telegram", "Polling mode enabled")
        # PTB's updater owns the getUpdates loop. This branch is kept for local
        # development; Render should use webhook mode to avoid 409 conflicts.
