...
def fetch_candles(symbol, interval="1min", outputsize=200):
    tf = normalize_custom_timeframe(interval)
    if tf in CUSTOM_TIMEFRAMES:
        source_size = max(250, int(outputsize * CUSTOM_TIMEFRAMES[tf] / 5) + 50)
        base = _provider_fetch_candles(symbol, "5min", outputsize=source_size)
        out = _resample_custom_ohlcv(base, CUSTOM_TIMEFRAMES[tf])
        return out.tail(outputsize).reset_index(drop=True)
    return _provider_fetch_candles(symbol, tf, outputsize=outputsize)
...
CUSTOM_TIMEFRAMES = {"35m": 35, "45m": 45, "1h": 60, "60m": 60}

def normalize_custom_timeframe(interval):
    x = str(interval).strip().lower()
    return {"35min": "35m", "45min": "45m", "1hour": "1h", "60min": "1h"}.get(x, x)

def _resample_custom_ohlcv(df, minutes):
    x = df.copy()
    x["datetime"] = pd.to_datetime(x["datetime"], utc=True)
    x = x.sort_values("datetime").set_index("datetime")
    out = x.resample(f"{minutes}min", label="left", closed="left").agg({
...
def power_confirmation(symbol, interval, primary):
    """Higher-timeframe agreement filter; conflicts become WAIT."""
    tf = normalize_custom_timeframe(interval)
    peers_map = {
        "1m": ["5m", "15m"], "5m": ["1m", "15m", "35m"],
        "15m": ["5m", "35m", "45m"], "35m": ["15m", "45m", "1h"],
        "45m": ["35m", "1h", "15m"], "1h": ["45m", "35m", "15m"]
    }
    peers = peers_map.get(tf, ["5m", "15m"])
    results = []
    for p in peers:
        try:
            d = fetch_candles(symbol, p, outputsize=120)
            if len(d) >= 60:
                results.append((p, make_signal(d)))
        except Exception:
            pass

    direction = primary.get("direction", "WAIT")
    if direction not in ("UP", "DOWN"):
        return {"quality": primary.get("confidence", 0), "agreement": "WAIT", "peers": []}

    same = sum(1 for _, r in results if r.get("direction") == direction)
    opp = sum(1 for _, r in results if r.get("direction") in ("UP", "DOWN") and r.get("direction") != direction)

    if opp:
        agreement = f"CONFLICT ({same}/{len(results)})"
        quality = min(59, int(primary.get("confidence", 0) * .65))
    elif same >= 2:
        agreement = f"CONFIRMED ({same}/{len(results)})"
        quality = min(99, int(primary.get("confidence", 0) * .75 + 25))
    else:
        agreement = f"PARTIAL ({same}/{len(results)})"
        quality = min(79, int(primary.get("confidence", 0) * .85))

    return {
        "quality": quality,
        "agreement": agreement,
        "peers": [{"timeframe": tf, "direction": r.get("direction"), "score": r.get("score")}
                  for tf, r in results]
    }
...
def calculate_signal(symbol, interval):
    ...
    s = make_signal(df)
    try:
        c = power_confirmation(symbol, interval, s)
        s["power_quality"] = c["quality"]
        s["mtf_agreement"] = c["agreement"]
        s["confirmation"] = c["peers"]
        if "CONFLICT" in c["agreement"] and s["direction"] in ("UP", "DOWN"):
            s["direction"] = "WAIT"
            s["reasons"] = list(s.get("reasons", [])) + ["Higher-timeframe conflict — WAIT"]
    except Exception:
        s["power_quality"] = s.get("confidence", 0)
...
def multi_timeframe_signal(symbol):
    ...
    """
    results = {}
    for tf in ["1min", "5min", "15min", "35m", "45m", "1h"]:
        try:
...
def build_help_text():
    return (
        ...
        "🧭 *Multi-Timeframe* — checks 1m, 5m, 15m, 35m, 45m and 1H together and shows "
        "whether they agree.\n\n"
...
async def tg_button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "m:mtf":
            await safe_edit(query, "Choose a pair for 1m+5m+15m check:", reply_markup=build_symbol_menu("mtf"))
            return
