...
TF_PROVIDER_PREFERENCE = {
    ...
    "1h": ["dukascopy", "twelvedata", "finnhub", "yahoo2", "yahoo", "biquote"],
...
PAIRS = {
    ...
    "AUD/EUR": {"finnhub": "OANDA:AUD_EUR", "yahoo": "AUDEUR=X", "td": "AUD/EUR"},
...
def premium_signal_api():
    ...
    allowed_pairs = {"AUD/CHF","AUD/EUR","AUD/JPY","AUD/CAD","AUD/USD","EUR/JPY","EUR/USD","GBP/USD"}
    allowed_tf = {"1m","5m","15m","30m","45m","1h","4h"}
...
def health():
    ...
    return {"status": "ok", "version": "4.8-all-timeframes", "bot_mode": BOT_MODE, "providers": provider,
...
def candles(pair, tf, limit=None):
    ...
    with fetch_lock:
        ...
        # Preferred higher-timeframe lane: derive from the validated 5m feed
        # before spending quota on a separate 15m/30m/45m/1h/4h endpoint.
        # This specifically fixes the case where 5m works but a higher-timeframe
        # endpoint is delayed or returns an incompatible cadence.
        if tf in {"15m", "30m", "45m", "1h", "4h"}:
            try:
...
def analyze_candles(candles,timeframe=None):
    cs=_v27_completed(candles)
    if len(cs)<60:
        return {"signal":"WAIT","direction":"WAIT","confidence":0,"score":0,
                "reason":"insufficient_completed_history"}
    o=[_v27_num(c.get("open")) for c in cs]
    h=[_v27_num(c.get("high")) for c in cs]
    l=[_v27_num(c.get("low")) for c in cs]
    cl=[_v27_num(c.get("close")) for c in cs]
    atr=_v27_atr(cs,14)
    if atr<=0:return {"signal":"WAIT","direction":"WAIT","confidence":0,"reason":"invalid_volatility"}

    e8=_v27_ema(cl[-50:],8);e21=_v27_ema(cl[-80:],21);e34=_v27_ema(cl[-100:],34)
    trend=.6*max(-1,min(1,(e8-e21)/(atr*1.4)))+.4*max(-1,min(1,(e21-e34)/(atr*2)))
    rs=[]
    for n in (2,3,5,8,13):
        r=(cl[-1]/cl[-1-n]-1) if cl[-1-n] else 0
        rs.append(max(-1,min(1,r*500)))
    momentum=.30*rs[0]+.25*rs[1]+.20*rs[2]+.15*rs[3]+.10*rs[4]

    rsi0=_v27_rsi(cl,14);rsi=max(-1,min(1,(rsi0-50)/18))
    if rsi0>74 or rsi0<26:rsi*=.72

    ss=[]
    for c in cs[-3:]:
        oo,hh,ll,cc=[_v27_num(c.get(k)) for k in ("open","high","low","close")]
        rr=max(hh-ll,1e-12);body=(cc-oo)/rr
        up=(hh-max(oo,cc))/rr;dn=(min(oo,cc)-ll)/rr
        s=max(-1,min(1,body*1.8))
        if body>0 and dn>up*1.25:s=min(1,s+.15)
        if body<0 and up>dn*1.25:s=max(-1,s-.15)
        ss.append(s)
    structure=.5*ss[-1]+.3*ss[-2]+.2*ss[-3]

    hi=max(h[-24:-1]);lo=min(l[-24:-1]);width=max(hi-lo,1e-12)
    breakout=.9 if cl[-1]>hi else -.9 if cl[-1]<lo else max(-1,min(1,(cl[-1]-(hi+lo)/2)/(width*.55)))

    support=min(l[-12:-1]);resistance=max(h[-12:-1]);sr=0
    if (resistance-cl[-1])/atr<.35:sr=-.45
    elif (cl[-1]-support)/atr<.35:sr=.45

    signs=[1 if cl[i]>cl[i-1] else -1 if cl[i]<cl[i-1] else 0 for i in range(len(cl)-12,len(cl))]
    persistence=sum(signs)/len(signs)

    a7=_v27_atr(cs,7);a28=_v27_atr(cs,28)
    vol=max(-1,min(1,(a7/a28-1))) if a28 else 0
    quality=1 if .45<=((h[-1]-l[-1])/atr)<=2.4 else .78

    score=(.22*trend+.18*momentum+.12*rsi+.13*structure+.12*breakout+
           .08*sr+.10*persistence+.05*vol)*quality
    direction="UP" if score>0 else "DOWN"
    factors=[trend,momentum,rsi,structure,breakout,sr,persistence]
    active=max(1,sum(abs(x)>=.05 for x in factors))
    agree=sum((x>0)==(score>0) for x in factors if abs(x)>=.05)
    ratio=agree/active
    if abs(score)<.075 or ratio<.57:
        return {"signal":"WAIT","direction":"WAIT","confidence":0,
                "score":round(score,4),"reason":"low_confluence"}
    confidence=max(51,min(94,round((50+abs(score)*48)*(.82+.18*ratio))))
    return {"signal":direction,"direction":direction,"confidence":confidence,
            "score":round(score,4),"reason":"validated_next_candle"}
