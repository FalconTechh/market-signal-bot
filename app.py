...
# ============================================================
# NexCandle AI v4.2 Advanced Next-Candle UP/DOWN Telegram Engine
# ============================================================
...
DATA_TIMEOUT = max(5, int(os.getenv("MARKET_DATA_TIMEOUT_SECONDS", "8")))
...
# Emergency resilience only: an expired in-memory candle cache may be reused
# when every provider is temporarily down.  The engine never fabricates prices
# or random directions.  A fresh SiftingIO websocket tick can update the last
# candle before analysis, so the result is as current as the live quote.
STALE_CACHE_MAX_AGE = max(60, int(os.getenv("STALE_CACHE_MAX_AGE", "900")))
LIVE_TICK_MAX_AGE = max(10, int(os.getenv("LIVE_TICK_MAX_AGE", "120")))
 ...
 <div class="layout">
  <section class="card">
   <div class="controls">
    ...
    <select id="tf" class="select"><option>1m</option><option selected>5m</option><option>15m</option><option>30m</option><option>1h</option></select>
  ...
  <aside class="card board">
   ...
   <button class="scanbtn full" onclick="scan()">⚡ FULL POWER SCAN</button>
...
async function getSignal(p){
 ...
 try{
  ...
  if(!j.ok || !j.signal || !['UP','DOWN'].includes(j.signal.display_direction)) throw new Error('signal');
...
async function scan(){
 ...
 try{
  ...
  hint.textContent=lastDirection ? 'LIVE DATA REFRESH • LAST VALID SIGNAL ACTIVE' : 'LIVE DATA LINK • RETRYING';
...
def get_directional_signal(pair, tf):
    """Advanced deterministic next-candle direction engine.

    Internally retains multi-factor indicators, AI ensemble evidence,
    multi-timeframe confirmation, freshness checks, caching and resilient
    provider fallback. The public Telegram Get Signal result is intentionally
    reduced to exactly one token: UP or DOWN.

    If full MTF consensus is unavailable, a fresh local multi-factor analysis
    is used. During a short provider outage, the last validated direction for
    the exact pair/timeframe may be reused. No random direction is generated.
    """
    key=(pair,tf)
    now=time.time()
    try:
        ...
        if direction not in {"CALL","PUT"}:
            x=candles(pair,tf)
            direction,score,why,x=analyse(x)
            if direction not in {"CALL","PUT"}:
                c=calc(x)
                a=c.iloc[-1]
                p=c.iloc[-2]
                ai=ai_ensemble(c)
                vote=float(ai.get("fusion",0.0))
                price_bias=1 if (a.close>p.close and a.close>=a.e21) else -1
                direction=("CALL" if ((vote>0.08 and price_bias>0) or vote>=0.22)
                           else ("PUT" if ((vote<-0.08 and price_bias<0) or vote<=-0.22)
                                 else ("CALL" if price_bias>0 else "PUT")))
            s=dict(s)
            s["direction"]=direction
            s["score"]=int(np.clip(s.get("score",score),1,99))
            s["target_candle"]="NEXT_CANDLE"
        with LOCK:
            LAST_DIRECTIONAL[key]=(now,direction,dict(s))
        return s, False
    except Exception as primary_exc:
        # Try one direct local-data pass before using the short-lived last result.
        try:
            x=candles(pair,tf)
            meta=market_meta(pair,tf,x)
            if not meta["fresh"]:
                raise RuntimeError("stale market data")
            direction,score,why,x=analyse(x)
            if direction not in {"CALL","PUT"}:
                c=calc(x)
                a=c.iloc[-1]
            ...
            s={
                "pair":pair,"tf":tf,"direction":direction,"score":int(np.clip(score,1,99)),
                "why":why,"provider":meta["provider"],"data_age":meta["age"],
                "entry":float(x.close.iloc[-1]),"rr":None,
                "state":"DIRECTIONAL FALLBACK","fresh":True,
            ...
            if old and now-old[0] <= LAST_DIRECTIONAL_MAX_AGE and old[1] in {"CALL","PUT"}:
                s=dict(old[2])
                s["direction"]=old[1]
                s["state"]="LAST VALIDATED"
...
def health_data():
    ...
    probes = []
    if SIFTING_API_KEY: probes.append(("sifting", _sifting_hist))
...
def _stale_cached_frame(key, pair, tf, limit):
    """Return a bounded emergency cache only when providers are temporarily down.

    This is intentionally conservative: no random/fabricated candle is created.
    If a fresh SiftingIO websocket tick exists, it is applied to the cached last
    candle as a live-quote overlay.
    """
    with LOCK:
        cached = MARKET_CACHE.get(key)
    if not cached:
        return None
    saved_at, frame, provider = cached
    if time.time() - saved_at > STALE_CACHE_MAX_AGE or frame is None or frame.empty:
        return None
    x = frame.copy()
    sym = _sifting_symbol(pair)
    with SIFTING_TICKS_LOCK:
        tick = dict(SIFTING_TICKS.get(sym) or {})
    ts = float(tick.get("ts") or 0)
    ...
    if price is None:
        ...
        if bid is not None and ask is not None:
            ...
            price = float(ask)
    if price is not None and ts and time.time() - ts <= LIVE_TICK_MAX_AGE:
        price = float(price)
        idx = x.index[-1]
        x.loc[idx, "close"] = price
        x.loc[idx, "high"] = max(float(x.loc[idx, "high"]), price)
        x.loc[idx, "low"] = min(float(x.loc[idx, "low"]), price)
...
def candles(pair, tf, limit=350):
    ...
    with fetch_lock:
        ...
        emergency = _stale_cached_frame(key, pair, tf, limit)
        if emergency is not None and len(emergency) >= 80:
            with LOCK:
                MARKET_CACHE[key] = (time.time(), emergency.copy(), "emergency-cache")
            db_log("warning", "market", f"{pair}/{tf}: provider outage; using bounded validated cache")
            return emergency
...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if d.startswith("run:"):
            ...
            direction = "UP" if s["direction"] == "CALL" else "DOWN"
            consume(uid)
