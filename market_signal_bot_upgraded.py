...
Install:
    pip install -r requirements.txt\n    # Optional stronger historical ML layer: pip install scikit-learn
...
# Optional ML layer. The bot remains functional without scikit-learn.
try:
...
ML_MIN_SAMPLES = int(os.getenv("ML_MIN_SAMPLES", "40"))
ML_MAX_ROWS = int(os.getenv("ML_MAX_ROWS", "1500"))
...
def ml_feature_vector_from_signal(s):
    price = float(s.get("price") or 0)
    if not price:
        price = 1.0
    return [
        float(s.get("rsi", 50)),
        float((s.get("ema9", price) - s.get("ema21", price)) / price * 1000),
        float((s.get("macd", 0) - s.get("macd_signal", 0)) / price * 1000),
        float(s.get("adx", 0)),
        float(s.get("atr", 0) / price * 100),
        float(s.get("stoch_k", 50)),
        float(s.get("stoch_d", 50)),
        float((s.get("bb_mid", price) - price) / price * 1000),
        float(s.get("score", 0)),
    ]


def train_historical_model(symbol, interval):
    """Train only from this bot's own resolved signal history."""
    if not ML_AVAILABLE:
        return None, 0
    ...
    with DB_LOCK:
        con = db()
        rows = con.execute("""
            SELECT price,rsi,ema9,ema21,macd,macd_signal,adx,atr,bb_mid,
                   stoch_k,stoch_d,score,result
            FROM signals
            WHERE symbol=? AND interval=?
              AND result IN ('WIN','LOSS')
              AND direction IN ('UP','DOWN','WEAK UP','WEAK DOWN')
            ORDER BY id DESC LIMIT ?
        """, (normalize_symbol(symbol), normalize_interval(interval), ML_MAX_ROWS)).fetchall()
        con.close()
    ...
    X, y = [], []
    for r in rows:
        X.append(ml_feature_vector_from_signal(dict(r)))
...
def ml_probability(symbol, interval, signal):
    """Historical setup probability; never presented as a guarantee."""
    model, samples = train_historical_model(symbol, interval)
    if model is None:
        return None, samples
    try:
        p = float(model.predict_proba(
            np.asarray([ml_feature_vector_from_signal(signal)])
        )[0][1] * 100)
        return round(p, 1), samples
    except Exception:
        return None, samples


def enhanced_quality(signal, ml_prob=None):
    signs = signal.get("tag_signs", [])
    raw_score = abs(float(signal.get("score", 0)))
    dominant = 1 if signal.get("score", 0) >= 0 else -1
    agreement = (
        sum(1 for _, sign in signs if sign == dominant) / len(signs) * 100
        if signs else 0
    )
    quality = 0.65 * raw_score + 0.35 * agreement
    if ml_prob is not None:
        # Blend only as supporting evidence.
        quality = 0.65 * quality + 0.35 * ml_prob
    return int(max(1, min(99, round(quality))))


def get_next_candle_timing(df, interval):
    tf = normalize_interval(interval)
    mins = INTERVAL_MINUTES.get(tf, 1)
    last = df["datetime"].iloc[-1]
    if last.tzinfo is None:
        last = last.tz_localize("UTC")
    next_close = last + pd.Timedelta(minutes=mins)
    return {
        "last_candle_close": last.isoformat(),
        "next_candle_close": next_close.isoformat(),
        "minutes": mins,
    }
...
def market_regime(df):
    ...
    if price and av / price * 100 < 0.03:
        return "LOW VOLATILITY"
    return "RANGE / MIXED"


def scan_all_symbols(interval="5m"):
    results = []
    for symbol in SYMBOLS.values():
        try:
            ...
            if len(df) < 60:
                continue
            s = make_signal(df)
            s["ml_probability"], s["ml_samples"] = ml_probability(symbol, interval, s)
            s["quality"] = enhanced_quality(s, s["ml_probability"])
            results.append({
                "symbol": normalize_symbol(symbol),
            ...
            continue
    return sorted(results, key=lambda x: (x["quality"], abs(x["score"])), reverse=True)
...
def make_signal(df: pd.DataFrame):
    ...
    return {
        ...
        "regime": market_regime(df),
        "next_candle": get_next_candle_timing(df, "1min"),
        "ml_probability": None,
        "ml_samples": 0,
        "quality": int(min(99, max(1, abs(score)))),
        "reasons": reasons,
        "tags": tags,
        "tag_signs": tag_signs,
    }
...
def calculate_signal(symbol, interval):
    ...
    # Historical ML is optional and uses only resolved signals from this bot.
    s["ml_probability"], s["ml_samples"] = ml_probability(
        normalized_symbol, normalized_interval, s
    )
    s["quality"] = enhanced_quality(s, s["ml_probability"])

    # Exact timing of the next candle window.
    s["next_candle"] = get_next_candle_timing(df, normalized_interval)

    s["id"] = save_signal(normalized_symbol, normalized_interval, s)
    return s
    ...
    for sym in syms:
        try:
            ...
            if len(df) < 60:
                continue
            s = make_signal(df)
            ml_p, ml_n = ml_probability(sym, normalize_interval(interval), s)
            quality = enhanced_quality(s, ml_p)
            results.append({
...
def format_signal(symbol, interval, s):
    ...
    header = (
        ...
        f"🧠 Signal quality: {s.get('quality', conf)}%\n"
        f"🤖 Historical ML edge: {str(s.get('ml_probability')) + '%' if s.get('ml_probability') is not None else 'Not enough history'}\n"
        f"📌 Market regime: {s.get('regime', 'UNKNOWN')}\n"
        f"📊 Trend: {trend_label}\n"
...
def main_menu_keyboard():
    """Full one-tap Telegram menu."""
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("📈 1M Signal", callback_data="sig|1m"),
            InlineKeyboardButton("📊 5M Signal", callback_data="sig|5m"),
        ],
        [
            InlineKeyboardButton("⏱ 15M Signal", callback_data="sig|15m"),
...
async def enhanced_callback(update, context):
    """Handle the added one-tap buttons."""
    ...
    if data.startswith("sig|"):
        ...
        allowed, reason, remaining = has_access(query.from_user.id, query.from_user.username)
        if not allowed:
            await query.edit_message_text(
                "🔒 Signal access expired. Use /start to view subscription options."
            )
            return
        try:
            s = calculate_signal(symbol, tf)
            if reason == "trial":
                consume_trial(query.from_user.id)
            await query.edit_message_text(
                format_signal(symbol, tf, s),
                reply_markup=main_menu_keyboard()
            )
        except Exception as e:
            await query.edit_message_text(
    ...
    if data == "mtf":
        symbol = context.user_data.get("symbol", "EUR/USD")
        try:
            result = multi_timeframe_signal(symbol)
            await query.edit_message_text(
                format_mtf(result), reply_markup=main_menu_keyboard()
            )
        except Exception as e:
            await query.edit_message_text(
                f"❌ MTF error: {e}", reply_markup=main_menu_keyboard()
            )
        return

    if data == "scan":
        try:
            rows = scan_all_symbols("5m")[:8]
            if not rows:
                msg = "No market data available."
            else:
                msg = "🔎 TOP MARKET SETUPS (5M)\\n\\n" + "\\n".join(
                    f"{i+1}. {r['symbol']} — {r['direction']} | Q {r['quality']}% | "
                    f"Score {r['score']:+d} | {r['regime']}"
                    for i, r in enumerate(rows)
                )
            await query.edit_message_text(msg, reply_markup=main_menu_keyboard())
        except Exception as e:
            await query.edit_message_text(
                f"❌ Scan error: {e}", reply_markup=main_menu_keyboard()
            )
        return

    if data == "best":
        try:
            rows = scan_all_symbols("5m")
            best = next((r for r in rows if r["direction"] in ("UP", "DOWN")), None)
            if not best:
                msg = "⚪ No strong setup right now."
            else:
                msg = (
                    "🔥 BEST CURRENT SETUP\\n\\n"
                    f"Asset: {best['symbol']}\\n"
                    ...
                    f"Historical ML edge: {best['ml_probability'] if best['ml_probability'] is not None else 'N/A'}\\n"
                    f"Regime: {best['regime']}\\n\\n"
                    "⚠️ Historical/model scores are not guarantees."
                )
            await query.edit_message_text(msg, reply_markup=main_menu_keyboard())
        except Exception as e:
            await query.edit_message_text(
                f"❌ Best-setup error: {e}", reply_markup=main_menu_keyboard()
            )
        return

    # Existing command functions can remain available; buttons below simply
    # point users to their familiar slash commands.
    command_map = {
        "history": "/history",
        "accuracy": "/accuracy",
        "backtest": "/backtest",
    ...
    if data in command_map:
        await query.edit_message_text(
            f"Use {command_map[data]} to open this section.",
            reply_markup=main_menu_keyboard()
        )
...
def build_main_menu():
    """
    Top-level categorized menu. Grouping keeps the list short and scannable
    instead of showing 11 buttons at once.
    """
    rows = [
        [InlineKeyboardButton("📊 Analysis Tools", callback_data="cat:analysis")],
        [InlineKeyboardButton("📁 My Account", callback_data="cat:account")],
        [InlineKeyboardButton("💎 Subscribe / Premium", callback_data="m:subscribe")],
        [InlineKeyboardButton("ℹ️ News & Help", callback_data="cat:info")],
    ]
