...
# ============================================================
# NexCandle AI v3.7 SiftingIO Advanced Market Data - production-oriented Telegram signal bot
# ============================================================
...
SIFTING_WS_ENABLED = (os.getenv("SIFTING_WS_ENABLED", "true").strip().lower() not in ("0", "false", "no", "off"))
...
<title>NexCandle AI — Premium Signal Scanner</title>
<style>
:root{--bg:#05070b;--panel:#0a1018;--line:#182633;--text:#eaf7f3;--muted:#718692;--up:#00ff8a;--down:#ff315d;--cyan:#00d9ff}
*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 50% 15%,#102b2a 0,#071016 28%,#05070b 65%);color:var(--text);font-family:Inter,system-ui,Arial,sans-serif;min-height:100vh;overflow-x:hidden}
body:before{content:"";position:fixed;inset:0;background:linear-gradient(rgba(0,255,180,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(0,255,180,.025) 1px,transparent 1px);background-size:42px 42px;mask-image:linear-gradient(to bottom,#000,transparent 80%);pointer-events:none}
.wrap{max-width:1150px;margin:auto;padding:18px}.top{display:flex;justify-content:space-between;align-items:center;padding:10px 2px 18px}.brand{font-weight:900;letter-spacing:.14em;font-size:14px}.live{font-size:10px;color:#9aabb3;border:1px solid #1b3037;padding:7px 10px;border-radius:999px}.live i{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--up);box-shadow:0 0 12px var(--up);margin-right:6px}
.grid{display:grid;grid-template-columns:1fr 360px;gap:18px}.hero,.side{background:linear-gradient(145deg,rgba(12,22,29,.94),rgba(5,10,15,.94));border:1px solid #17303a;border-radius:28px;box-shadow:0 25px 80px rgba(0,0,0,.5),inset 0 1px rgba(255,255,255,.035);overflow:hidden}.hero{min-height:650px;position:relative}.herohead{display:flex;gap:8px;padding:18px;border-bottom:1px solid #13252e}.select{background:#081017;color:#d8e8e6;border:1px solid #203841;border-radius:12px;padding:10px 12px;outline:0}.scanner{height:420px;display:grid;place-items:center;position:relative;perspective:900px}.orb{width:245px;height:245px;border-radius:50%;position:relative;transform-style:preserve-3d;animation:float 4s ease-in-out infinite}.orb:before{content:"";position:absolute;inset:22px;border-radius:50%;background:radial-gradient(circle at 38% 32%,#b7fff0 0,#00ff9d 7%,#073d35 30%,#06121a 68%);box-shadow:0 0 30px var(--up),0 0 100px rgba(0,255,138,.2),inset 0 0 45px #00ff9d;animation:pulse 1.8s infinite}.orb:after{content:"";position:absolute;inset:-30px;border:2px solid rgba(0,255,138,.55);border-radius:50%;box-shadow:0 0 25px rgba(0,255,138,.45);animation:spin 3s linear infinite}.ring{position:absolute;border:1px solid rgba(0,217,255,.5);border-radius:50%;transform:rotateX(70deg) rotateZ(15deg);animation:spin 5s linear infinite}.r1{inset:-5px}.r2{inset:-42px;animation-duration:8s;animation-direction:reverse}.r3{inset:40px;animation-duration:4s}.scan{position:absolute;width:360px;height:360px;border-radius:50%;border-top:2px solid var(--cyan);animation:scan 2.4s linear infinite;filter:drop-shadow(0 0 12px var(--cyan));opacity:.7}.label{position:absolute;text-align:center;z-index:3;text-shadow:0 0 18px currentColor}.label .word{font-size:58px;font-weight:1000;letter-spacing:.12em}.label .sub{font-size:10px;letter-spacing:.3em;color:#b2c4c8;margin-top:7px}.status{position:absolute;bottom:15px;text-align:center;font-size:11px;letter-spacing:.22em;color:#90a7ad}.direction-up{color:var(--up)}.direction-down{color:var(--down)}
.info{display:grid;grid-template-columns:repeat(4,1fr);gap:1px;border-top:1px solid #14262f}.metric{padding:17px;background:#081017}.metric span{display:block;color:#60767f;font-size:9px;letter-spacing:.15em;margin-bottom:7px}.metric b{font-size:17px}.bar{height:5px;background:#102028;border-radius:99px;overflow:hidden;margin-top:8px}.bar i{display:block;height:100%;background:currentColor;border-radius:99px;box-shadow:0 0 12px currentColor}
.side{padding:18px}.side h3{margin:0 0 15px;font-size:12px;letter-spacing:.15em}.pairs{display:grid;gap:9px}.pair{padding:13px;border:1px solid #18303a;border-radius:15px;background:#081017;display:flex;justify-content:space-between;align-items:center;cursor:pointer}.pair:hover{border-color:#2c5a63;transform:translateY(-1px)}.pair small{display:block;color:#637a83;margin-top:3px}.badge{font-weight:900;font-size:11px;padding:7px 9px;border-radius:10px}.up{color:var(--up);background:rgba(0,255,138,.08)}.down{color:var(--down);background:rgba(255,49,93,.08)}.footer{margin-top:15px;color:#60757d;font-size:10px;line-height:1.6;text-align:center}.scanbtn{width:100%;margin-top:15px;border:1px solid #1e6455;background:linear-gradient(180deg,#0c3b32,#09231f);color:#dffff5;border-radius:14px;padding:14px;font-weight:900;letter-spacing:.15em;cursor:pointer;box-shadow:0 0 30px rgba(0,255,138,.08)}.scanbtn:active{transform:scale(.99)}
@keyframes spin{to{transform:rotate(360deg)}}@keyframes scan{to{transform:rotate(360deg)}}@keyframes pulse{50%{transform:scale(1.04);box-shadow:0 0 50px var(--up),0 0 130px rgba(0,255,138,.25),inset 0 0 55px #00ff9d}}@keyframes float{50%{transform:translateY(-9px) rotateY(7deg)}}
@media(max-width:850px){.grid{grid-template-columns:1fr}.side{order:2}.hero{min-height:590px}.info{grid-template-columns:repeat(2,1fr)}.scanner{height:390px}.orb{width:205px;height:205px}.scan{width:310px;height:310px}}
</style></head>
<body><div class="wrap"><div class="top"><div class="brand">NEXCANDLE AI <span style="color:#00ff8a">/ PREMIUM</span></div><div class="live"><i></i>LIVE MARKET ENGINE</div></div>
<div class="grid"><section class="hero"><div class="herohead"><select id="pair" class="select"><option>EUR/USD</option><option>GBP/USD</option><option>USD/JPY</option><option>AUD/USD</option><option>USD/CAD</option><option>USD/CHF</option><option>NZD/USD</option><option>EUR/JPY</option><option>GBP/JPY</option></select><select id="tf" class="select"><option>1m</option><option selected>5m</option><option>15m</option><option>30m</option><option>1h</option></select><button class="scanbtn" style="margin:0 0 0 auto;width:auto;padding:10px 15px" onclick="scan()">SCAN</button></div>
<div class="scanner"><div class="orb"><div class="ring r1"></div><div class="ring r2"></div><div class="ring r3"></div></div><div class="scan"></div><div class="label" id="label"><div class="word">SCANNING</div><div class="sub">AI MARKET ANALYSIS</div></div><div class="status" id="status">VALIDATING LIVE DATA • MULTI-TIMEFRAME • AI FUSION</div></div>
<div class="info"><div class="metric"><span>DIRECTION</span><b id="dir">—</b></div><div class="metric"><span>SETUP SCORE</span><b id="score">—</b><div class="bar"><i id="scorebar" style="width:0%"></i></div></div><div class="metric"><span>PRICE</span><b id="price">—</b></div><div class="metric"><span>R:R</span><b id="rr">—</b></div></div></section>
<aside class="side"><h3>PREMIUM MARKET BOARD</h3><div class="pairs" id="pairs"></div><button class="scanbtn" onclick="scan()">⚡ FULL POWER SCAN</button><div class="footer">UP/DOWN is the model's directional output from validated market data. It is not a guaranteed outcome or profit promise.</div></aside></div></div>
<script>
const pairs=['EUR/USD','GBP/USD','USD/JPY','AUD/USD','USD/CAD','EUR/JPY'];
function cls(d){return d==='UP'?'up':'down'}
async function get(p){const r=await fetch('/api/premium-signal?pair='+encodeURIComponent(p)+'&tf='+document.querySelector('#tf').value);return await r.json()}
async function scan(){
 const label=document.querySelector('#label'),status=document.querySelector('#status'); label.innerHTML='<div class="word">SCANNING</div><div class="sub">3D AI ENGINE ACTIVE</div>';status.textContent='SCANNING • STRUCTURE • MOMENTUM • MTF • LIVE DATA';
 try{const j=await get(document.querySelector('#pair').value);if(!j.ok)throw 0;const s=j.signal;const d=s.display_direction;label.className='label '+(d==='UP'?'direction-up':'direction-down');label.innerHTML='<div class="word">'+d+'</div><div class="sub">SIGNAL READY • '+s.display_score+'/100</div>';status.textContent='SCAN COMPLETE • '+s.provider.toUpperCase()+' • '+s.display_pair+' • '+s.display_tf;document.querySelector('#dir').textContent=d;document.querySelector('#dir').className=d==='UP'?'direction-up':'direction-down';document.querySelector('#score').textContent=s.display_score+'/100';document.querySelector('#scorebar').style.width=s.display_score+'%';document.querySelector('#price').textContent=s.entry?Number(s.entry).toFixed(5):'—';document.querySelector('#rr').textContent=s.rr?s.rr.toFixed(2):'—';}
 catch(e){status.textContent='DATA RETRY • KEEPING SCANNER ACTIVE';setTimeout(scan,2500)}
}
async function board(){const box=document.querySelector('#pairs');box.innerHTML='';for(const p of pairs){const el=document.createElement('div');el.className='pair';el.innerHTML='<div><b>'+p+'</b><small>AI scan</small></div><span class="badge">...</span>';box.appendChild(el);get(p).then(j=>{if(j.ok){const d=j.signal.display_direction;el.querySelector('.badge').textContent=d;el.querySelector('.badge').className='badge '+cls(d)}else el.querySelector('.badge').textContent='ERR'}).catch(()=>{})}}
board();scan();setInterval(scan,60000);
</script></body></html>"""

@web.get("/")
def home():
    return Response(PREMIUM_HTML, mimetype="text/html")

@web.get("/api/premium-signal")
def premium_signal_api():
    pair = request.args.get("pair", "EUR/USD").upper().replace("-", "/")
    tf = request.args.get("tf", "5m")
    allowed_pairs = {"EUR/USD","GBP/USD","USD/JPY","AUD/USD","USD/CAD","USD/CHF","NZD/USD","EUR/JPY","GBP/JPY"}
    allowed_tf = {"1m","5m","15m","30m","1h"}
    ...
    try:
        ...
        # The public premium screen is intentionally directional: UP/DOWN only.
        # When the strict confluence engine returns WAIT, use the latest validated
        # local directional bias rather than pretending a high-confidence setup exists.
        direction = s.get("direction")
        if direction == "WAIT":
            x = candles(pair, tf)
            direction, local_score, why, _ = analyse(x)
            direction = direction if direction in {"CALL", "PUT"} else "CALL"
            s["score"] = max(50, int(round(local_score)))
            s["why"] = why
            s["state"] = "DIRECTIONAL"
            s["entry"] = float(x.close.iloc[-1])
            v = max(float(x.atr.iloc[-1]), float(x.close.iloc[-1]) * 0.00008)
            risk, reward = 1.15*v, 1.85*v
            if direction == "CALL":
                s["stop"], s["target"] = s["entry"]-risk, s["entry"]+reward
            else:
                s["stop"], s["target"] = s["entry"]+risk, s["entry"]-reward
            s["rr"] = reward/risk
        s["display_direction"] = "UP" if direction == "CALL" else "DOWN"
        s["display_score"] = int(max(1, min(99, s.get("score", 0))))
        s["display_pair"] = pair
        s["display_tf"] = tf
...
def _sifting_hist(pair, tf, limit):
    ...
    # Add headroom because FX weekends/market gaps mean the requested number of
    # rows is not always available in a simple wall-clock window.
    days = max(2, int((limit * base_minutes * 1.8) / 1440) + 2)
    end = datetime.now(timezone.utc)
    start = end - timedelta(days=days)
    url = f"https://api.sifting.io/v1/hist/forex/{_sifting_symbol(pair)}/bars"
    r = S.get(url, headers={"X-API-Key": SIFTING_API_KEY, "Accept-Encoding":"gzip"}, params={
        "start": start.strftime("%Y-%m-%d"), "end": end.strftime("%Y-%m-%dT%H:%M:%SZ"), "interval": base_tf
    ...
    try:
        ...
        raise RuntimeError(f"SiftingIO invalid JSON: {exc}") from exc
...
def _sifting_ws_loop():
    if not SIFTING_API_KEY or websocket is None or not SIFTING_WS_ENABLED:
        return
    backoff=1
...
def candles(pair, tf, limit=350):
    ...
    with fetch_lock:
        ...
        providers = []
        if SIFTING_API_KEY:
            providers.append(("sifting", _sifting_hist))
        if OANDA_API_TOKEN:
...
def provider_status():
    now = time.time(); out=[]
    configured=[("SiftingIO", "sifting", SIFTING_API_KEY), ("OANDA", "oanda", OANDA_API_TOKEN), ("TwelveData", "twelvedata", TWELVEDATA_API_KEY), ("Finnhub", "finnhub", FINNHUB_API_KEY), ("Alpha Vantage", "alphavantage", ALPHAVANTAGE_API_KEY), ("Yahoo fallback", "yahoo", True)]
